...
renderer_type: "view"
renderer_sources: ["views/receipt.html"]
view_sources: ["views/receipt.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-09/build09-workspaces-contract.test.mjs","tests/build-09/build09r2-bespoke-contract.test.mjs","tests/build-09/cross-domain-lifecycles.test.mjs","tests/build-09/dock-crossdock-domain.test.mjs","tests/build-09/expiration-queue-browser.test.mjs","tests/build-09/mobile-receiving-domain.test.mjs","tests/build-09/mobile-receiving-picking-browser.test.mjs","tests/build-09/operational-32-page-matrix-chromium.test.mjs"]
...
- Route: switchPage('receipt') / views/receipt.html
- Page type: TRANSACTION

# 2. Business Purpose

Receipts — finance (risk: medium, phase: core)
...
User opens this page to work with the Receipt workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/receipt.html. Static source counts at baseline: 3 headings, 5 button tags, 17 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/receipt.html
...
- UI → renderer: views/receipt.html
...
- UI label: إضافة بند; handler/action ID: addReceiptLineItem(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: تحديث الوصل; handler/action ID: updateReceiptPreview(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: حفظ وتسجيل الحركة; handler/action ID: saveReceiptFromForm(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: طباعة; handler/action ID: printFinanceReceipt(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Receipt record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Canonical candidate: `receipt` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `receipt` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Receipt workflow; the current 231/231 click audit is route activation evidence only.
...
- Receipt workspace presentation and its explicitly verified page-specific actions: addFinanceDemoData(, addReceiptLineItem(), updateReceiptPreview(), saveReceiptFromForm(), printFinanceReceipt().
