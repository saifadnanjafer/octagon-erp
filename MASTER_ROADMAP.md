...
2. Pick the top unfinished item in Section 4 (build top to bottom). Currently:
   **Phase 6 Audit / tenant hardening** — run the button-by-button launch audit, dead-code/performance pass, release notes, and deeper legacy backfill policy. The old v6 receivable mapping #21 is already fixed (see note in §6), the multi-company tenant-control slice is shipped in `multi_entity`, V5 service-layer enforcement is shipped in `tenant_service`, and direct persistence endpoint enforcement is shipped in `server_api_tenant`; browser Route Health proof is green as of 2026-06-14. Remaining tenant work is policy-level: decide how aggressive legacy companyId backfill should be, and continue the broader launch audit/performance/release-readiness pass. Always run **فحص صحة النظام** first; target **66/66 nav, 66/66 pages** (new modules `assets`, `subscriptions`, `people_ops`, `helpdesk`, `fleet`, `documents`, `marketing`, `budgeting`, `procurement`, `projects`, `approvals`, `field_service`, `rental` raised it from 53), 8/8 globals, 14/14 fns, 13/13 cols, 0 link orphans. Then run **🏭 فحص استقرار الورشة** (in جاهزية التشغيل) — target 12/12 (backup may warn).
