---
page_id: "downtime_board"
title_en: "Downtime Board"
title_ar: ""
domain: "ops"
navigation_group: "ops_inventory"
kind: DASHBOARD
canonical_status: "PRIMARY"
canonical_home: "downtime_board"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
tables_or_entities: ["shopfloor_downtime_events", "shopfloor_sessions"]
...
- Page ID: `downtime_board`; English: Downtime Board; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('downtime_board')`; page-specific renderer is `modules/build09-downtime-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Log active downtime against a shop-floor session, classify the reason, end it, and preserve duration/history.
...
- Data loading: `downtime, shopfloor-sessions` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: shopfloor:downtime_start, shopfloor:downtime_end.
...
- Renderer/domain source: `modules/build09-downtime-workspace.js` → `performance.listDowntimeEvents and start/end downtime handlers`.
- Persisted entities/tables: shopfloor_downtime_events, shopfloor_sessions.
...
- `shopfloor:downtime_start`
- `shopfloor:downtime_end`
...
- The page renders scoped records from `downtime, shopfloor-sessions`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into shopfloor_downtime_events, shopfloor_sessions and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: shopfloor_terminal, workcenter_queue.
- Downstream: operational_performance, maintenance proposal boundary.
...
- English label: Downtime Board; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-downtime_board-GAP-001** (P1, WORKFLOW) — maintenance_required is a proposal only and must not be documented as maintenance creation
- **PAGE-downtime_board-GAP-002** (P1, TEST) — live duration, end event, and filter behavior need direct browser evidence
...
- Canonical home: `downtime_board` unless a later owner decision explicitly changes the navigation contract.
...
- `performance.listDowntimeEvents and start` → `end downtime handlers`
...
- Downtime Board page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
