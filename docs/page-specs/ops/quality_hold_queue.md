...
domain_sources: ["modules/build09-quality-workspace.js", "platform/wms/index.mjs", "platform/quality/operations.mjs"]
tables_or_entities: ["quality_checkpoints", "quality_dispositions", "quality_ncrs"]
test_sources: ["tests/build-09/quality-workspaces-browser.test.mjs", "tests/build-09/quality-rework-scrap-domain.test.mjs", "tests/build-09/operational-production-quality-chromium.test.mjs"]
...
- Page ID: `quality_hold_queue`; English: Quality Hold Queue; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('quality_hold_queue')`; page-specific renderer is `modules/build09-quality-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Review failed/held checkpoints, synchronize canonical inspection state, conditionally accept with a second actor, or request a disposition.
...
- Renderer: `modules/build09-quality-workspace.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `quality-checkpoints` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: quality:checkpoint_sync, quality:checkpoint_conditional_accept, quality:disposition_request.
...
- Renderer/domain source: `modules/build09-quality-workspace.js` → `qualityOperations.listOperationalCheckpoints and checkpoint sync/conditional accept/disposition request`.
- Persisted entities/tables: quality_checkpoints, quality_dispositions, quality_ncrs.
...
- `quality:checkpoint_sync`
- `quality:checkpoint_conditional_accept`
- `quality:disposition_request`
...
- The page renders scoped records from `quality-checkpoints`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into quality_checkpoints, quality_dispositions, quality_ncrs and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: receiving, shopfloor_terminal, production_receipt.
...
- English label: Quality Hold Queue; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-quality_hold_queue-GAP-001** (P1, PERMISSION) — conditional acceptance and disposition request have maker-checker constraints requiring browser proof
- **PAGE-quality_hold_queue-GAP-002** (P1, ERROR_STATE) — stale canonical checkpoint and missing NCR must be visible as distinct errors
...
- Canonical home: `quality_hold_queue` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-quality-workspace.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/quality-workspaces-browser.test.mjs`
- `tests/build-09/quality-rework-scrap-domain.test.mjs`
...
- `qualityOperations.listOperationalCheckpoints and checkpoint sync` → `conditional accept` → `disposition request`
...
- Quality Hold Queue page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
