...
domain_sources: ["modules/build09-receiving-discrepancy-workspace.js", "platform/wms/index.mjs", "platform/manufacturing/downtime-performance.mjs"]
tables_or_entities: ["wms_receiving_discrepancies", "wms_receiving_lines", "wms_receiving_sessions"]
test_sources: ["tests/build-09/receiving-discrepancy-browser.test.mjs", "tests/build-09/mobile-receiving-domain.test.mjs", "tests/build-09/operational-32-page-matrix-chromium.test.mjs"]
...
- Page ID: `receiving_discrepancies`; English: Receiving Discrepancies; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('receiving_discrepancies')`; page-specific renderer is `modules/build09-receiving-discrepancy-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Review quantity, damage, lot, or reference discrepancies and accept or reject the affected receiving line.
...
- Renderer: `modules/build09-receiving-discrepancy-workspace.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `receiving-discrepancies` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:receiving_discrepancy_approve.
...
- Renderer/domain source: `modules/build09-receiving-discrepancy-workspace.js` → `receiving.listReceivingDiscrepancies/approveReceivingDiscrepancy`.
- Persisted entities/tables: wms_receiving_discrepancies, wms_receiving_lines, wms_receiving_sessions.
...
- The page renders scoped records from `receiving-discrepancies`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_receiving_discrepancies, wms_receiving_lines, wms_receiving_sessions and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: mobile_receiving.
- Downstream: receiving session review and canonical posting.
...
- English label: Receiving Discrepancies; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-receiving_discrepancies-GAP-001** (P1, PERMISSION) — approval must enforce reviewer authority and warehouse scope server-side
- **PAGE-receiving_discrepancies-GAP-002** (P1, ERROR_STATE) — the UI needs direct proof for stale, already-decided, and missing-line responses
...
- Canonical home: `receiving_discrepancies` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-receiving-discrepancy-workspace.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/receiving-discrepancy-browser.test.mjs`
...
- `receiving.listReceivingDiscrepancies` → `approveReceivingDiscrepancy`
...
- Receiving Discrepancies page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
