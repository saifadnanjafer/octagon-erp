...
renderer_type: "view"
renderer_sources: ["views/workshop_command_center.html"]
view_sources: ["views/workshop_command_center.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/workshop/browser-flows.test.mjs","tests/workshop/browser-harness.mjs","tests/workshop/command-center-contract.test.mjs","tests/workshop/command-center-domain.test.mjs","tests/workshop/drilldown-contract.test.mjs","tests/workshop/pilot-role-boundaries.test.mjs","tests/workshop/readiness-contract.test.mjs"]
...
- Page ID: `workshop_command_center`
- English: Workshop Command Center
- Arabic: مركز قيادة الورشة
- Domain: `ops`; navigation group: `ops_control`
- Route: switchPage('workshop_command_center') / views/workshop_command_center.html
- Page type: DASHBOARD

# 2. Business Purpose

Workshop Command Center — workshop/operations (risk: medium, phase: internal-workshop-wave)

# 3. Primary Users

- Declared roles: workshop.user, workshop.manager, finance.manager
- Viewer/operator/reviewer/manager/administrator split: NOT VERIFIED beyond the declared permission gate.

# 4. Primary User Goal

User opens this page to work with the Workshop Command Center workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/workshop_command_center.html. Static source counts at baseline: 2 headings, 3 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/workshop_command_center.html
- Visible action inventory: Refresh; &times;; Open canonical workspace
...
- UI → renderer: views/workshop_command_center.html
...
- UI label: Refresh; action ID/API/domain handler/persistence: NOT VERIFIED by the baseline inventory.
- UI label: &times; action ID/API/domain handler/persistence: NOT VERIFIED by the baseline inventory.
- UI label: Open canonical workspace; action ID/API/domain handler/persistence: NOT VERIFIED by the baseline inventory.
- UI label: Refresh; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: &times;; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: Open canonical workspace; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.

# 8. Inputs

- Static controls: Refresh; &times;; Open canonical workspace
...
The intended output is the page’s named Workshop Command Center record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Client page gate: workshop.user, workshop.manager, finance.manager
- Entitlement: none/global (not a commercial/SaaS-gated page)
...
- Arabic label: مركز قيادة الورشة
- English label: Workshop Command Center
...
- Canonical candidate: `workshop_command_center` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `workshop_command_center` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Workshop Command Center workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/workshop/browser-flows.test.mjs, tests/workshop/browser-harness.mjs, tests/workshop/command-center-contract.test.mjs, tests/workshop/command-center-domain.test.mjs, tests/workshop/drilldown-contract.test.mjs, tests/workshop/pilot-role-boundaries.test.mjs, tests/workshop/readiness-contract.test.mjs
...
- Workshop Command Center workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
