---
page_id: "fleet_device_mapping"
title_en: "Fleet Device Mapping"
title_ar: "ربط الأسطول بالأجهزة"
domain: "resources"
navigation_group: "resources_fleet"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "fleet_device_mapping"
parent_page: null
aliases: []
...
- Page ID: `fleet_device_mapping`
- English: Fleet Device Mapping
- Arabic: ربط الأسطول بالأجهزة
- Domain: `resources`; navigation group: `resources_fleet`
- Route: switchPage('fleet_device_mapping') / views/fleet_device_mapping.html
- Page type: PAGE

# 2. Business Purpose

Fleet device mapping — fleet/devices (risk: medium, phase: build10)
...
User opens this page to work with the Fleet Device Mapping workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/fleet_device_mapping.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Fleet Device Mapping record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: ربط الأسطول بالأجهزة
- English label: Fleet Device Mapping
...
- Canonical candidate: `fleet_device_mapping` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `fleet_device_mapping` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Fleet Device Mapping workflow; the current 231/231 click audit is route activation evidence only.
...
- Fleet Device Mapping workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
