...
page_id: "equipment"
title_en: "Equipment"
title_ar: "معدات الورشة"
domain: "ops"
navigation_group: "ops_inventory"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "equipment"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
renderer_type: "view"
renderer_sources: ["views/equipment.html"]
view_sources: ["views/equipment.html"]
api_sources: []
...
- Route: switchPage('equipment') / views/equipment.html
- Page type: PAGE

# 2. Business Purpose

Equipment — workshop/production (risk: medium, phase: core)
...
User opens this page to work with the Equipment workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/equipment.html. Static source counts at baseline: 1 headings, 3 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/equipment.html
...
- UI → renderer: views/equipment.html
...
- UI label: ➕ إضافة معدة; handler/action ID: showAddEquipmentModal(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ✈️ ترحيل جديد (خارجي); handler/action ID: showDispatchModal(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: 📋 تشييك شهري سريع; handler/action ID: runMonthlyEquipmentAudit(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Equipment record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: معدات الورشة
- English label: Equipment
...
- Canonical candidate: `equipment` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `equipment` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Equipment workflow; the current 231/231 click audit is route activation evidence only.
...
- Equipment workspace presentation and its explicitly verified page-specific actions: showAddEquipmentModal(), showDispatchModal(), runMonthlyEquipmentAudit().
