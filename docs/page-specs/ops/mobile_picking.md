---
page_id: "mobile_picking"
title_en: "Mobile Picking"
title_ar: "الالتقاط المتنقل"
domain: "ops"
navigation_group: "ops_warehouse_fulfillment"
kind: KIOSK
canonical_status: PRIMARY
canonical_home: "mobile_picking"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
renderer_sources: ["modules/build09-mobile-picking.js"]
view_sources: []
api_sources: ["/api/v1/action/${actionId}","/api/v1/wms/pick-tasks?warehouse_id=${encodeURIComponent"]
domain_sources: ["modules/build09-mobile-picking.js","services/permissionService.js","modules/build09-pick-task-queue-workspace.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-09/build09-workspaces-contract.test.mjs","tests/build-09/build09r2-bespoke-contract.test.mjs","tests/build-09/mobile-receiving-picking-browser.test.mjs","tests/build-09/operational-32-page-matrix-chromium.test.mjs"]
...
- Page ID: `mobile_picking`
- English: Mobile Picking
- Arabic: الالتقاط المتنقل
- Domain: `ops`; navigation group: `ops_warehouse_fulfillment`
- Route: switchPage('mobile_picking') / views/mobile_picking.html
- Page type: KIOSK

# 2. Business Purpose

Mobile picking — workshop/inventory (risk: high, phase: build09)
...
User opens this page to work with the Mobile Picking workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/mobile_picking.html. Static source counts at baseline: 2 headings, 14 button tags, 12 input/select/textarea controls, 0 tables, 5 forms, and 2 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: modules/build09-mobile-picking.js
...
- UI → renderer: modules/build09-mobile-picking.js
- Renderer → API/query: /api/v1/action/${actionId}, /api/v1/wms/pick-tasks?warehouse_id=${encodeURIComponent
...
- UI label: ${escapeHtml(task.sourceDocumentId || task.id)} · ${escapeHtml(task.productId)} ${escapeHtml(String(task.requestedQuantity))} ${escapeHtml(task.assignedTo ? task.status : (rtl() ? 'غير مسند' : 'unassigned'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ↻ ${rtl() ? 'تحديث الطابور' : 'Refresh queue'}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${rtl() ? 'تأكيد المصدر' : 'Confirm source'}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${rtl() ? 'تأكيد المنتج' : 'Confirm product'}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${rtl() ? 'تأكيد الالتقاط' : 'Confirm pick'}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
- UI label: ${rtl() ? 'تجهيز' : 'Stage'}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
- UI label: ${rtl() ? 'العودة إلى الطابور' : 'Back to queue'}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(row.assignedTo ? t('Reassign picker', 'إعادة إسناد الملتقط') : t('Assign picker', 'إسناد ملتقط'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Open Mobile Picking', 'فتح الالتقاط المتنقل'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Mobile Picking record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.

# 10. Workflow Position

- Upstream: NOT VERIFIED from explicit workflow metadata.
- Downstream: `pick_task_queue`
...
- Arabic label: الالتقاط المتنقل
- English label: Mobile Picking
...
- Canonical candidate: `mobile_picking` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `mobile_picking` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Mobile Picking workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-09/build09-workspaces-contract.test.mjs, tests/build-09/build09r2-bespoke-contract.test.mjs, tests/build-09/mobile-receiving-picking-browser.test.mjs, tests/build-09/operational-32-page-matrix-chromium.test.mjs
...
- Mobile Picking workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
