...
renderer_sources: ["views/approvals.html","modules/build12-workspaces.js"]
...
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-12/build12-acceptance-contract.test.mjs","tests/checkpoint-c/procurement_lifecycle.test.mjs","tests/checkpoint-g/canonical_cutover_controller.test.mjs","tests/phase02/browser-live-evidence.test.mjs","tests/phase02/runtime-strangler.test.mjs","tests/phase02/security-suite.test.mjs","tests/phase02/workflow-approvals.test.mjs","tests/phase03/finance-browser-evidence.test.mjs"]
...
- Route: switchPage('approvals') / views/approvals.html
- Page type: PAGE

# 2. Business Purpose

Approvals — manager/approval (risk: high, phase: phase6e)
...
User opens this page to work with the Approvals workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/approvals.html. Static source counts at baseline: 3 headings, 3 button tags, 2 input/select/textarea controls, 1 tables, 1 forms, and 2 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/approvals.html
...
- UI → renderer: views/approvals.html
...
The intended output is the page’s named Approvals record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: الموافقات
- English label: Approvals
...
- Canonical candidate: `approvals` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `approvals` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Approvals workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-12/build12-acceptance-contract.test.mjs, tests/checkpoint-c/procurement_lifecycle.test.mjs, tests/checkpoint-g/canonical_cutover_controller.test.mjs, tests/phase02/browser-live-evidence.test.mjs, tests/phase02/runtime-strangler.test.mjs, tests/phase02/security-suite.test.mjs, tests/phase02/workflow-approvals.test.mjs, tests/phase03/finance-browser-evidence.test.mjs
...
- Approvals workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
