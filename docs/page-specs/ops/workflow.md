...
renderer_type: "view"
renderer_sources: ["views/workflow.html"]
view_sources: ["views/workflow.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-08/planning-browser-chromium.test.mjs","tests/checkpoint-c/migration_048.test.mjs","tests/checkpoint-g/failure_injection_complete.test.mjs","tests/phase02/browser-live-evidence.test.mjs","tests/phase02/collaboration-files-jobs.test.mjs","tests/phase02/runtime-strangler.test.mjs","tests/phase02/security-suite.test.mjs","tests/phase02/workflow-approvals.test.mjs"]
...
- Page ID: `workflow`
- English: Workflow
- Arabic: مصمم العمليات
- Domain: `ops`; navigation group: `ops_control`
- Route: switchPage('workflow') / views/workflow.html
- Page type: PAGE

# 2. Business Purpose

Workflow designer — workshop/operations (risk: medium, phase: core)
...
User opens this page to work with the Workflow workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/workflow.html. Static source counts at baseline: 2 headings, 5 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/workflow.html
...
- UI → renderer: views/workflow.html
...
- UI label: اختبر وتحقّق; handler/action ID: runWorkflowSimulation(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: نشر; handler/action ID: publishWorkflow(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: تنفيذ سير العمل; handler/action ID: triggerWorkflowExecution(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: إضافة عقدة; handler/action ID: addWorkflowNode(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: إلغاء الربط; handler/action ID: clearWorkflowSelection(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Workflow record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: مصمم العمليات
- English label: Workflow
...
- Canonical candidate: `workflow` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `workflow` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Workflow workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-08/planning-browser-chromium.test.mjs, tests/checkpoint-c/migration_048.test.mjs, tests/checkpoint-g/failure_injection_complete.test.mjs, tests/phase02/browser-live-evidence.test.mjs, tests/phase02/collaboration-files-jobs.test.mjs, tests/phase02/runtime-strangler.test.mjs, tests/phase02/security-suite.test.mjs, tests/phase02/workflow-approvals.test.mjs
...
- Workflow workspace presentation and its explicitly verified page-specific actions: runWorkflowSimulation(), publishWorkflow(), triggerWorkflowExecution(), addWorkflowNode(), clearWorkflowSelection().
