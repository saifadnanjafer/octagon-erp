---
page_id: "variance_review"
title_en: "Variance Review"
title_ar: ""
domain: "ops"
navigation_group: "ops_inventory"
kind: PAGE
canonical_status: "PRIMARY"
canonical_home: "variance_review"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
tables_or_entities: ["wms_count_sessions_v2", "wms_count_lines_v2"]
test_sources: ["tests/build-09/cycle-counting-domain.test.mjs", "tests/build-09/count-workspaces-browser.test.mjs", "tests/build-09/cross-domain-lifecycles.test.mjs"]
...
- Page ID: `variance_review`; English: Variance Review; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('variance_review')`; page-specific renderer is `modules/build09-count-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Review count variance, request a recount or approve it, then request and acknowledge a canonical stock adjustment.
...
- Data loading: `count-sessions` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:count_approve_variance, wms:count_recount, wms:count_request_adjustment.
...
- Renderer/domain source: `modules/build09-count-workspace.js` → `cycleCounting.requestRecount/approveCountVariance/requestCountAdjustment/acknowledgeCountAdjustment`.
- Persisted entities/tables: wms_count_sessions_v2, wms_count_lines_v2.
...
- `wms:count_approve_variance`
- `wms:count_recount`
- `wms:count_request_adjustment`
...
- The page renders scoped records from `count-sessions`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_count_sessions_v2, wms_count_lines_v2 and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: count_session.
- Downstream: canonical inventory adjustment.
...
- English label: Variance Review; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-variance_review-GAP-001** (P1, PERMISSION) — maker-checker separation for variance approval must be visible in the spec and browser evidence
- **PAGE-variance_review-GAP-002** (P1, WORKFLOW) — adjustment acknowledgement must verify every canonical result ID before closing the session
...
- Canonical home: `variance_review` unless a later owner decision explicitly changes the navigation contract.
...
- `cycleCounting.requestRecount` → `approveCountVariance` → `requestCountAdjustment` → `acknowledgeCountAdjustment`
...
- Variance Review page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
