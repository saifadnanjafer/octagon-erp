---
page_id: "offline_client_registry"
title_en: "Offline Client Registry"
title_ar: "سجل عملاء دون اتصال"
domain: "resources"
navigation_group: "resources_offline"
kind: SETUP
canonical_status: PRIMARY
canonical_home: "offline_client_registry"
parent_page: null
aliases: []
...
- Page ID: `offline_client_registry`
- English: Offline Client Registry
- Arabic: سجل عملاء دون اتصال
- Domain: `resources`; navigation group: `resources_offline`
- Route: switchPage('offline_client_registry') / views/offline_client_registry.html
- Page type: SETUP

# 2. Business Purpose

Offline client registry — offline/clients (risk: high, phase: build10)
...
User opens this page to work with the Offline Client Registry workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/offline_client_registry.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Offline Client Registry record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: سجل عملاء دون اتصال
- English label: Offline Client Registry
...
- Canonical candidate: `offline_client_registry` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `offline_client_registry` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Offline Client Registry workflow; the current 231/231 click audit is route activation evidence only.
...
- Offline Client Registry workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
