...
renderer_type: "view"
renderer_sources: ["views/import.html"]
view_sources: ["views/import.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/autopilot/autopilot-validation.test.mjs","tests/build-01/rma-foundation.test.mjs","tests/build-02/contracts-authority.test.mjs","tests/build-03/commissions-warranty.test.mjs","tests/build-04/integration-hub-rendering.test.mjs","tests/build-05/platform-services-browser.test.mjs","tests/build-05/platform-services-lifecycle.test.mjs","tests/build-05/platform-services-runtime.test.mjs"]
...
- Route: switchPage('import') / views/import.html
- Page type: PAGE

# 2. Business Purpose

Attendance import — hr/payroll (risk: high, phase: phase6e)
...
User opens this page to work with the Import workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/import.html. Static source counts at baseline: 5 headings, 4 button tags, 3 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/import.html
...
- UI label: سحب الحقول بالذكاء الاصطناعي; handler/action ID: smartMapImportFields(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ابدأ المعالجة 🚀; handler/action ID: uploadTimesheet(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: الحفظ والتحويل للتايم شيت الذكي; handler/action ID: saveImportedData(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Import record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Canonical candidate: `import` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `import` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Import workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/autopilot/autopilot-validation.test.mjs, tests/build-01/rma-foundation.test.mjs, tests/build-02/contracts-authority.test.mjs, tests/build-03/commissions-warranty.test.mjs, tests/build-04/integration-hub-rendering.test.mjs, tests/build-05/platform-services-browser.test.mjs, tests/build-05/platform-services-lifecycle.test.mjs, tests/build-05/platform-services-runtime.test.mjs
...
- Import workspace presentation and its explicitly verified page-specific actions: smartMapImportFields(), uploadTimesheet(), saveImportedData().
