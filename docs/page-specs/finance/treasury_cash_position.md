...
- Page ID: `treasury_cash_position`; English: Treasury Cash Position; domain: `finance`; navigation group: `finance_treasury`.
- Route: `switchPage('treasury_cash_position')`; view: `views/treasury_cash_position.html`; renderer evidence: `app.js`.
- Canonical status: PRIMARY; this page is a primary navigation destination, not an embedded tab.

# 2. Business Purpose

A treasury manager opens Cash Position to view current cash by account/company/currency and reconcile the figure to posted Finance facts.
...
USER OPENS THIS PAGE TO: A treasury manager opens Cash Position to view current cash by account/company/currency and reconcile the figure to posted Finance facts.

# 5. AS-IS Runtime Surface

- View: `views/treasury_cash_position.html`; renderer: `app.js generic/legacy surface`.
- API/query boundary: GET /api/v1/finance/*; treasury page-specific route NOT VERIFIED.
- AS-IS classification: **THIN**; usability: **THIN**.
- Primary actions:
- Refresh/filter/export cash position -> page API/persistence NOT VERIFIED — permission: server authorization required; persistence: Treasury planning domain owns forecasts, funding proposals, facilities, alerts, and cash position views; Finance owns posted cash facts.
...
A treasury manager opens Cash Position to view current cash by account/company/currency and reconcile the figure to posted Finance facts. Expected output: scoped records, decision state, durable source reference, report, alert, or device/session state as appropriate. Exact persisted result is NOT VERIFIED for the page-specific surface.
...
- English: Treasury Cash Position; Arabic: · مركز النقد.
...
A treasury manager opens Cash Position to view current cash by account/company/currency and reconcile the figure to posted Finance facts. The target contract is a scoped, page-specific workflow that loads authoritative data, exposes lifecycle-valid actions, persists through **Treasury planning domain owns forecasts, funding proposals, facilities, alerts, and cash position views; Finance owns posted cash facts.**, produces a durable result/reference, and makes missing, denied, stale, validation, and server-error states explicit.

# 17. Identified Gaps

- **treasury_cash_position-W4-01** (P1, WORKFLOW_DISCONNECT) — Treasury domain sources exist, but the page-level link from forecast/proposal to an authorized payment or Finance result is not verified. **Required next step:** Define proposal approval, execution, and source-fact references.
- **treasury_cash_position-W4-02** (P2, CREDIBILITY_GAP) — Freshness, currency, company scope, and missing-data behavior are not established for the individual page. **Required next step:** Add explicit as-of, currency, stale-data, and empty states.
...
- For this page: A treasury manager opens Cash Position to view current cash by account/company/currency and reconcile the figure to posted Finance facts.
