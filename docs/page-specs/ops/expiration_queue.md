...
domain_sources: ["modules/build09-expiration-workspace.js", "platform/wms/index.mjs", "platform/wms/traceability-ops.mjs"]
tables_or_entities: ["wms_trace_profiles", "stock_lots", "stock_serials"]
test_sources: ["tests/build-09/expiration-queue-browser.test.mjs", "tests/build-09/traceability-recall-domain.test.mjs"]
...
- Page ID: `expiration_queue`; English: Expiration Queue; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('expiration_queue')`; page-specific renderer is `modules/build09-expiration-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Prioritize lots/serials approaching expiry and record a governed quality disposition without inventing stock state.
...
- Renderer: `modules/build09-expiration-workspace.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `expiration-queue` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:trace_quality_set.
...
- Renderer/domain source: `modules/build09-expiration-workspace.js` → `traceability.expirationQueue/setTraceQualityStatus`.
- Persisted entities/tables: wms_trace_profiles, stock_lots, stock_serials.
...
- The page renders scoped records from `expiration-queue`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_trace_profiles, stock_lots, stock_serials and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: lot_serial_traceability, receiving.
- Downstream: quality_hold_queue, recall_analysis.
...
- English label: Expiration Queue; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-expiration_queue-GAP-001** (P1, PERMISSION) — quality status changes require maker-checker and must be proven in the browser
- **PAGE-expiration_queue-GAP-002** (P1, DATA) — expiry thresholds/timezone and missing-expiry handling need explicit fixture evidence
...
- Canonical home: `expiration_queue` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-expiration-workspace.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/expiration-queue-browser.test.mjs`
...
- `traceability.expirationQueue` → `setTraceQualityStatus`
...
- Expiration Queue page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
