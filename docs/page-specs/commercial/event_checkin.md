---
page_id: "event_checkin"
title_en: "Event Check-in"
title_ar: "تسجيل حضور الفعالية"
domain: "commercial"
navigation_group: "commercial_marketing"
kind: TRANSACTION
canonical_status: PRIMARY
canonical_home: "event_checkin"
parent_page: null
aliases: []
...
- Page ID: `event_checkin`
- English: Event Check-in
- Arabic: تسجيل حضور الفعالية
- Domain: `commercial`; navigation group: `commercial_marketing`
- Route: switchPage('event_checkin') / views/event_checkin.html
- Page type: TRANSACTION

# 2. Business Purpose

Event check-in — build12/checkin (risk: high, phase: build12)
...
User opens this page to work with the Event Check-in workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/event_checkin.html. Static source counts at baseline: 2 headings, 3 button tags, 2 input/select/textarea controls, 1 tables, 1 forms, and 2 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Event Check-in record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: تسجيل حضور الفعالية
- English label: Event Check-in
...
The Event Check-in workspace should give its governed users a page-specific way to complete the named business task: load scoped records, accept governed inputs where needed, execute authorized actions, persist the resulting state through the domain authority, and expose explicit loading/empty/denied/validation/server-error/success states. Exact fields and lifecycle transitions remain **NOT VERIFIED** where the baseline source does not define them.

# 17. Identified Gaps

- **PAGE-event_checkin-GAP-004** (P1, PERMISSION) — The review inventory flags this commercial/SaaS page as lacking a matching entitlement mapping in the disposable review database.
- **PAGE-event_checkin-GAP-005** (P2, TEST) — Documentation contradiction: NAVIGATION_FORENSIC_REPORT.json records app.js switchPage, while the module registry also contains this page. Reconcile renderer ownership after Product Recovery.
...
- Canonical candidate: `event_checkin` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `event_checkin` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Event Check-in workflow; the current 231/231 click audit is route activation evidence only.
...
- Event Check-in workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
