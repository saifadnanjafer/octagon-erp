...
domain_sources: ["modules/build09-mobile-picking.js", "platform/wms/index.mjs", "platform/wms/picking.mjs"]
tables_or_entities: ["wms_pick_tasks_v2", "wms_warehouse_tasks"]
test_sources: ["tests/build-09/mobile-picking-wave-domain.test.mjs", "tests/build-09/mobile-receiving-picking-browser.test.mjs", "tests/build-09/pick-task-warehouse-isolation.test.mjs", "tests/build-09/operational-browser-chromium.test.mjs"]
...
- Page ID: `mobile_picking`; English: Mobile Picking; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('mobile_picking')`; page-specific renderer is `modules/build09-mobile-picking.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Select an eligible task, scan source and product/lot/serial, confirm quantity or short reason, stage, and hand off the stock move.
...
- Renderer: `modules/build09-mobile-picking.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `GET /api/v1/wms/pick-tasks?warehouse_id=...` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:pick_task_assign, wms:pick_scan_source, wms:pick_scan_product, wms:pick_confirm, wms:pick_stage, wms:pick_request_post, wms:pick_acknowledge_post.
...
- Renderer/domain source: `modules/build09-mobile-picking.js` → `picking.listPickTasks/assignPickTask/scanPickSource/scanPickProduct/confirmPick/stagePick/requestPickPost/acknowledgePickPost`.
- Persisted entities/tables: wms_pick_tasks_v2, wms_warehouse_tasks.
...
- `wms:pick_scan_source`
- `wms:pick_scan_product`
- `wms:pick_confirm`
- `wms:pick_stage`
- `wms:pick_request_post`
- `wms:pick_acknowledge_post`
...
- The page renders scoped records from `GET /api/v1/wms/pick-tasks?warehouse_id=...`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_pick_tasks_v2, wms_warehouse_tasks and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: pick_task_queue, wave_execution.
- Downstream: canonical inventory stock move, traceability.
...
- English label: Mobile Picking; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-mobile_picking-GAP-001** (P1, PERMISSION) — queue filtering and assignment must not expose another warehouse
- **PAGE-mobile_picking-GAP-002** (P1, WORKFLOW) — a direct Chromium path must prove short-pick exception and canonical acknowledgement, not only scanning contracts
...
- Canonical home: `mobile_picking` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-mobile-picking.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/mobile-picking-wave-domain.test.mjs`
...
- `picking.listPickTasks` → `assignPickTask` → `scanPickSource` → `scanPickProduct` → `confirmPick` → `stagePick` → `requestPickPost` → `acknowledgePickPost`
...
- Mobile Picking page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
