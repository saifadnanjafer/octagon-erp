...
export function listBuild12(db, ctx, resource, recordId, query = {}) {
  ...
  // Every scoped() call below previously omitted tenantId, shifting each
  // positional argument left by one: the order string landed in the tenantId
  // slot and the limit number landed in the order slot. scoped()'s template
  // literal interpolates that value straight into ORDER BY, so passing a
  // number there (e.g. ORDER BY 20) is a valid SQLite ordinal column
  // reference to a column that does not exist -- SQLite throws, and every
  // caller of this combined overview (ai_overview, marketing_overview,
  // people_development_overview) saw a 500. The people_skills call took a
  // different, quieter path: 'name' silently became the tenantId filter
  // instead of throwing, so it always returned zero rows for every tenant.
  if (resource === 'overview' || resource === 'ai-overview') return { data: { generated_at: timestamp(ctx), simulator: 'deterministic-v1', ai: { providers: rows(db,'SELECT * FROM ai_providers ORDER BY display_name'), tasks: rows(db,'SELECT * FROM ai_tasks ORDER BY task_id'), recent_runs: scoped(db,'ai_runs',tenantId,'created_at DESC',20), proposals: scoped(db,'ai_proposals',tenantId,'created_at DESC',20) }, people: { skills: scoped(db,'people_skills',tenantId,'name'), plans: scoped(db,'people_development_plans',tenantId,'updated_at DESC',20), certification_warnings: rows(db,"SELECT * FROM people_certifications WHERE tenant_id=? AND status='active' AND expires_at<=datetime('now','+30 day') ORDER BY expires_at",tenantId) }, marketing: { campaigns: scoped(db,'marketing_campaigns',tenantId,'updated_at DESC',20), content: scoped(db,'marketing_content',tenantId,'updated_at DESC',20) }, events: scoped(db,'build12_events',tenantId,'starts_at',20), pack: row(db,'SELECT * FROM build12_pack_profiles WHERE package_id=\'pack:al_warsha\'') } };
  ...
  // Per-resource order-column overrides for the two tables in `map` that
  // don't have a created_at column (people_competencies only has updated_at;
  // build12_event_registrations only has registered_at) -- the 'events'
  // substring check below doesn't catch 'event-registrations' or
  // 'event-sessions' (singular 'event', not 'events'), so both used to fall
  // through to `ORDER BY created_at DESC` on a column that doesn't exist and
  // throw a 500 on every request. event-sessions has both created_at and
  // starts_at so it was unaffected, but list it explicitly anyway so this
  // table's real column is never left to an accidental substring match.
  const orderOverride = { 'people-competencies': 'updated_at DESC', 'event-registrations': 'registered_at DESC', 'event-sessions': 'starts_at' };
  if (map[resource]) {
    ...
    return { data: read(map[resource], orderOverride[resource] || (resource.includes('events') ? 'starts_at' : 'created_at DESC')) };
  }
