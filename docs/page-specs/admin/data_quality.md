...
title_ar: "جودة البيانات"
domain: "admin"
navigation_group: "admin_org"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "data_quality"
parent_page: null
aliases: []
roles: ["system.admin"]
...
renderer_type: "view"
renderer_sources: ["views/data_quality.html"]
view_sources: ["views/data_quality.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-10/iot-registry-domain.test.mjs"]
...
- Page ID: `data_quality`
- English: Data Quality
- Arabic: جودة البيانات
- Domain: `admin`; navigation group: `admin_org`
- Route: switchPage('data_quality') / views/data_quality.html
- Page type: PAGE

# 2. Business Purpose

Data quality — admin/data (risk: high, phase: phase6e)
...
User opens this page to work with the Data Quality workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/data_quality.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/data_quality.html
...
- UI → renderer: views/data_quality.html
...
The intended output is the page’s named Data Quality record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: جودة البيانات
- English label: Data Quality
...
- **PAGE-data_quality-GAP-002** (P1, ACTION) — No meaningful primary action path was verified in the inspected source.
- **PAGE-data_quality-GAP-003** (P1, API) — No literal backend API path was verified in the inspected page-specific source; server capability remains NOT VERIFIED.
...
- Canonical candidate: `data_quality` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `data_quality` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Data Quality workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-10/iot-registry-domain.test.mjs
...
- Data Quality workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
