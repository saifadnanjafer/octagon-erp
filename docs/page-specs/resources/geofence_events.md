---
page_id: "geofence_events"
title_en: "Geofence Events"
title_ar: "أحداث السياج الجغرافي"
domain: "resources"
navigation_group: "resources_fleet"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "geofence_events"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
- Page ID: `geofence_events`
- English: Geofence Events
- Arabic: أحداث السياج الجغرافي
- Domain: `resources`; navigation group: `resources_fleet`
- Route: switchPage('geofence_events') / views/geofence_events.html
- Page type: PAGE

# 2. Business Purpose

Geofence events — fleet/geofences (risk: medium, phase: build10)
...
User opens this page to work with the Geofence Events workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/geofence_events.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Geofence Events record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: أحداث السياج الجغرافي
- English label: Geofence Events
...
- Canonical candidate: `geofence_events` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `geofence_events` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Geofence Events workflow; the current 231/231 click audit is route activation evidence only.
...
- Geofence Events workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
