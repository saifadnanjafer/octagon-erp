...
tables_or_entities: ["shopfloor_material_requests", "shopfloor_material_request_lines", "stock_moves"]
test_sources: ["tests/build-09/production-material-workspaces-browser.test.mjs", "tests/build-09/production-material-flow-domain.test.mjs", "tests/build-09/operational-production-quality-chromium.test.mjs"]
...
- Page ID: `production_receipt`; English: Production Receipt; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('production_receipt')`; page-specific renderer is `modules/build09-production-material-workspaces.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Review production output receipt proposals, request canonical Inventory posting, and expose traceability after acknowledgement.
...
- Renderer/domain source: `modules/build09-production-material-workspaces.js` → `materialFlow receipt proposals and canonical acknowledgement`.
- Persisted entities/tables: shopfloor_material_requests, shopfloor_material_request_lines, stock_moves.
...
- Mutations persist into shopfloor_material_requests, shopfloor_material_request_lines, stock_moves and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: shopfloor_terminal, quality_hold_queue.
...
- English label: Production Receipt; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-production_receipt-GAP-001** (P1, WORKFLOW) — quality verification is consumed, not owned, and the page must not imply it posts stock directly
- **PAGE-production_receipt-GAP-002** (P1, TEST) — prove canonical result reference appears after acknowledgement
...
- Canonical home: `production_receipt` unless a later owner decision explicitly changes the navigation contract.
...
- `materialFlow receipt proposals and canonical acknowledgement`
...
- Production Receipt page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
