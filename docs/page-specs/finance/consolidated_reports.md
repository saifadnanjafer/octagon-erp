...
renderer_type: "view"
renderer_sources: ["views/consolidated_reports.html"]
view_sources: ["views/consolidated_reports.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-08/build08-page-contract.test.mjs","tests/build-08/group-finance-browser-chromium.test.mjs"]
...
- Page ID: `consolidated_reports`
- English: Consolidated Reports
- Arabic: · التقارير الموحدة
- Domain: `finance`; navigation group: `finance_consolidation`
- Route: switchPage('consolidated_reports') / views/consolidated_reports.html
- Page type: ANALYTICS

# 2. Business Purpose

Consolidated reports — finance/consolidation (risk: high, phase: build08)

# 3. Primary Users

- Declared roles: finance.manager
- Viewer/operator/reviewer/manager/administrator split: NOT VERIFIED beyond the declared permission gate.

# 4. Primary User Goal

User opens this page to work with the Consolidated Reports workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/consolidated_reports.html. Static source counts at baseline: 1 headings, 0 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/consolidated_reports.html
...
- UI → renderer: views/consolidated_reports.html
...
The intended output is the page’s named Consolidated Reports record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Client page gate: finance.manager
- Entitlement: none/global (not a commercial/SaaS-gated page)
...
- Arabic label: · التقارير الموحدة
- English label: Consolidated Reports
...
- Canonical candidate: `consolidated_reports` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `consolidated_reports` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Consolidated Reports workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-08/build08-page-contract.test.mjs, tests/build-08/group-finance-browser-chromium.test.mjs
...
- Consolidated Reports workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
