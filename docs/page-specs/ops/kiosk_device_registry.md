---
page_id: "kiosk_device_registry"
title_en: "Kiosk Device Registry"
title_ar: "سجل أجهزة الأكشاك"
domain: "ops"
navigation_group: "ops_kiosks"
kind: "SETUP"
canonical_status: "PRIMARY"
canonical_home: "kiosk_device_registry"
parent_page: null
aliases: []
...
- Page ID: `kiosk_device_registry`; English: Kiosk Device Registry; domain: `ops`; navigation group: `ops_kiosks`.
- Route: `switchPage('kiosk_device_registry')`; view: `views/kiosk.html`; renderer evidence: `app.js`.
- Canonical status: PRIMARY; this page is a primary navigation destination, not an embedded tab.

# 2. Business Purpose

An administrator opens Kiosk Device Registry to register, revoke, and inspect device identity, entitlement, and offline-client status.

# 3. Primary Users

- Declared client gate: workshop.manager.
...
USER OPENS THIS PAGE TO: An administrator opens Kiosk Device Registry to register, revoke, and inspect device identity, entitlement, and offline-client status.
...
- Primary actions:
- Register/revoke/rotate device -> kiosk-registry action boundary; exact page route NOT VERIFIED — permission: server authorization required; persistence: Kiosk registry and service entitlement/signature domains own device/session/access facts; operational boards consume them.
...
- English: Kiosk Device Registry; Arabic: سجل أجهزة الأكشاك.
...
An administrator opens Kiosk Device Registry to register, revoke, and inspect device identity, entitlement, and offline-client status. The target contract is a scoped, page-specific workflow that loads authoritative data, exposes lifecycle-valid actions, persists through **Kiosk registry and service entitlement/signature domains own device/session/access facts; operational boards consume them.**, produces a durable result/reference, and makes missing, denied, stale, validation, and server-error states explicit.

# 17. Identified Gaps

- **kiosk_device_registry-W4-01** (P1, WORKFLOW_DISCONNECT) — Kiosk domain/tests prove operational lifecycle components, but the selected page-level service route and downstream work completion are not attributed as one flow. **Required next step:** Prove register/entitle/offline/restore and one service action handoff.
...
- For this page: An administrator opens Kiosk Device Registry to register, revoke, and inspect device identity, entitlement, and offline-client status.
