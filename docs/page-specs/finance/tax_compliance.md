...
renderer_type: "view"
renderer_sources: ["views/tax_compliance.html"]
...
- Page ID: `tax_compliance`
- English: Tax Compliance
- Arabic: الضرائب والفوترة
- Domain: `finance`; navigation group: `finance_accounts`
- Route: switchPage('tax_compliance') / views/tax_compliance.html
- Page type: PAGE

# 2. Business Purpose

Tax compliance — finance/compliance (risk: high, phase: phase6b)

# 3. Primary Users

- Declared roles: finance.manager, system.admin
- Viewer/operator/reviewer/manager/administrator split: NOT VERIFIED beyond the declared permission gate.

# 4. Primary User Goal

User opens this page to work with the Tax Compliance workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/tax_compliance.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/tax_compliance.html
...
- UI → renderer: views/tax_compliance.html
...
The intended output is the page’s named Tax Compliance record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Client page gate: finance.manager, system.admin
- Entitlement: none/global (not a commercial/SaaS-gated page)
...
- Arabic label: الضرائب والفوترة
- English label: Tax Compliance
...
- Canonical candidate: `tax_compliance` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `tax_compliance` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Tax Compliance workflow; the current 231/231 click audit is route activation evidence only.
...
- Tax Compliance workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
