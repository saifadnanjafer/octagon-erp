...
renderer_type: "view"
renderer_sources: ["views/customers.html"]
view_sources: ["views/customers.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-09/trace-workspaces-browser.test.mjs","tests/build-12/build12-domain.test.mjs","tests/checkpoint-f/cross_domain_record_integrity.test.mjs","tests/checkpoint-g/canonical_cutover_controller.test.mjs","tests/checkpoint-h/http_legacy_writer_refusal.test.mjs","tests/phase03/finance-ui-parity.test.mjs","tests/phase04/legacy_migration.test.mjs","tests/phase04/opening_cutover_phase04.test.mjs"]
...
- Page ID: `customers`
- English: Customers
- Arabic: أرصدة العملاء
- Domain: `finance`; navigation group: `finance_accounts`
- Route: switchPage('customers') / views/customers.html
- Page type: LIST

# 2. Business Purpose

Customer balances — finance (risk: medium, phase: core)
...
User opens this page to work with the Customers workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/customers.html. Static source counts at baseline: 3 headings, 3 button tags, 11 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/customers.html
...
- UI → renderer: views/customers.html
...
- UI label: إضافة العميل; handler/action ID: addCustomerFromForm(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: إضافة على الرصيد; handler/action ID: addCustomerCharge(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Customers record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: أرصدة العملاء
- English label: Customers
...
- Canonical candidate: `customers` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `customers` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Customers workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-09/trace-workspaces-browser.test.mjs, tests/build-12/build12-domain.test.mjs, tests/checkpoint-f/cross_domain_record_integrity.test.mjs, tests/checkpoint-g/canonical_cutover_controller.test.mjs, tests/checkpoint-h/http_legacy_writer_refusal.test.mjs, tests/phase03/finance-ui-parity.test.mjs, tests/phase04/legacy_migration.test.mjs, tests/phase04/opening_cutover_phase04.test.mjs
...
- Customers workspace presentation and its explicitly verified page-specific actions: addFinanceDemoData(, addCustomerFromForm(), addCustomerCharge().
