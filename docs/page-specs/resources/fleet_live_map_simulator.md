---
page_id: "fleet_live_map_simulator"
title_en: "Fleet Live Map Simulator"
title_ar: "محاكي الخريطة الحية للأسطول"
domain: "resources"
navigation_group: "resources_fleet"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "fleet_live_map_simulator"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
- Page ID: `fleet_live_map_simulator`
- English: Fleet Live Map Simulator
- Arabic: محاكي الخريطة الحية للأسطول
- Domain: `resources`; navigation group: `resources_fleet`
- Route: switchPage('fleet_live_map_simulator') / views/fleet_live_map_simulator.html
- Page type: PAGE

# 2. Business Purpose

Fleet live map simulator — fleet/map (risk: low, phase: build10)
...
User opens this page to work with the Fleet Live Map Simulator workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/fleet_live_map_simulator.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Fleet Live Map Simulator record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: محاكي الخريطة الحية للأسطول
- English label: Fleet Live Map Simulator
...
- Canonical candidate: `fleet_live_map_simulator` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `fleet_live_map_simulator` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Fleet Live Map Simulator workflow; the current 231/231 click audit is route activation evidence only.
...
- Fleet Live Map Simulator workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
