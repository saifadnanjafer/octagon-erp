---
page_id: "conflict_resolution"
title_en: "Conflict Resolution"
title_ar: "حل التعارضات"
domain: "resources"
navigation_group: "resources_offline"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "conflict_resolution"
parent_page: null
aliases: []
...
- Page ID: `conflict_resolution`
- English: Conflict Resolution
- Arabic: حل التعارضات
- Domain: `resources`; navigation group: `resources_offline`
- Route: switchPage('conflict_resolution') / views/conflict_resolution.html
- Page type: PAGE

# 2. Business Purpose

Conflict resolution — offline/conflicts (risk: high, phase: build10)
...
User opens this page to work with the Conflict Resolution workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/conflict_resolution.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Conflict Resolution record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: حل التعارضات
- English label: Conflict Resolution
...
- Canonical candidate: `conflict_resolution` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `conflict_resolution` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Conflict Resolution workflow; the current 231/231 click audit is route activation evidence only.
...
- Conflict Resolution workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
