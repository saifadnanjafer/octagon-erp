---
page_id: "receiving_discrepancies"
title_en: "Receiving Discrepancies"
title_ar: "فروقات الاستلام"
domain: "ops"
navigation_group: "ops_warehouse_inbound"
kind: QUEUE
canonical_status: PRIMARY
canonical_home: "receiving_discrepancies"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
renderer_sources: ["modules/build09-receiving-discrepancy-workspace.js"]
view_sources: []
api_sources: []
domain_sources: ["modules/build09-receiving-discrepancy-workspace.js","services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-09/build09-workspaces-contract.test.mjs","tests/build-09/build09r2-bespoke-contract.test.mjs","tests/build-09/operational-32-page-matrix-chromium.test.mjs","tests/build-09/receiving-discrepancy-browser.test.mjs","tests/workshop/pilot-exceptions.test.mjs"]
...
- Page ID: `receiving_discrepancies`
- English: Receiving Discrepancies
- Arabic: فروقات الاستلام
- Domain: `ops`; navigation group: `ops_warehouse_inbound`
- Route: switchPage('receiving_discrepancies') / views/receiving_discrepancies.html
- Page type: QUEUE

# 2. Business Purpose

Receiving discrepancies — workshop/inventory (risk: high, phase: build09)
...
User opens this page to work with the Receiving Discrepancies workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/receiving_discrepancies.html. Static source counts at baseline: 3 headings, 2 button tags, 1 input/select/textarea controls, 0 tables, 1 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: modules/build09-receiving-discrepancy-workspace.js
...
- UI → renderer: modules/build09-receiving-discrepancy-workspace.js
...
- UI label: ${esc(t('Record governed decision', 'تسجيل قرار محكوم'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(`${label(row, 'type', 'discrepancy_type')} · ${label(row, 'sessionId', 'session_id')}`)} ${badge(label(row, 'status', 'status'), tone[label(row, 'status', 'status')] || '')} ${esc(t('Expected', 'المتوقع'))}: ${esc(label(row, 'expectedValue', 'expected_value') || '—')} · ${esc(t('Actual', 'الفعلي'))}: ${esc(label(row, 'actualValue', 'actual_value') || '—')}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Receiving Discrepancies record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: فروقات الاستلام
- English label: Receiving Discrepancies
...
- Canonical candidate: `receiving_discrepancies` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `receiving_discrepancies` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Receiving Discrepancies workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-09/build09-workspaces-contract.test.mjs, tests/build-09/build09r2-bespoke-contract.test.mjs, tests/build-09/operational-32-page-matrix-chromium.test.mjs, tests/build-09/receiving-discrepancy-browser.test.mjs, tests/workshop/pilot-exceptions.test.mjs
...
- Receiving Discrepancies workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED.
