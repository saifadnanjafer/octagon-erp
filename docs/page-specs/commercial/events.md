...
renderer_sources: ["views/events.html","modules/build12-workspaces.js"]
view_sources: ["views/events.html"]
api_sources: ["/api/v1/build12/","/api/v1/build12/${resource}","/api/v1/action/${id}","/api/v1/iot/${pageKey}","/api/v1/iot/fleet/${pageKey}","/api/v1/iot/offline/${pageKey}","/api/v1/iot/kiosk/${pageKey}","/api/v1/iot/boards/${pageKey}","/api/v1/action/${pageKey}"]
domain_sources: ["services/permissionService.js","modules/build12-workspaces.js","modules/build10-workspaces.js","modules/build09-downtime-workspace.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-09/downtime-workspaces-browser.test.mjs","tests/build-10/build10-workspaces-contract.test.mjs","tests/build-10/cross-domain-scenarios.test.mjs","tests/build-10/fleet-telematics-geofences.test.mjs","tests/build-10/iot-offline-kiosk-export-contract.test.mjs","tests/build-10/iot-telemetry-health-commands.test.mjs","tests/build-11/commercial-platform-browser.test.mjs","tests/build-11/commercial-platform.test.mjs"]
...
- Route: switchPage('events') / views/events.html
- Page type: PAGE

# 2. Business Purpose

Events Hub — workshop/operations (risk: medium, phase: phase6h)
...
User opens this page to work with the Events workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/events.html. Static source counts at baseline: 14 headings, 10 button tags, 5 input/select/textarea controls, 2 tables, 2 forms, and 8 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/events.html
...
- UI → renderer: views/events.html
- Renderer → API/query: /api/v1/build12/, /api/v1/build12/${resource}, /api/v1/action/${id}, /api/v1/iot/${pageKey}, /api/v1/iot/fleet/${pageKey}, /api/v1/iot/offline/${pageKey}, /api/v1/iot/kiosk/${pageKey}, /api/v1/iot/boards/${pageKey}, /api/v1/action/${pageKey}
...
- UI label: ${esc(t('End downtime', 'إنهاء التوقف'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Start downtime', 'بدء التوقف'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Events record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.

# 10. Workflow Position

- Upstream: NOT VERIFIED from explicit workflow metadata.
- Downstream: `downtime_board`, `operational_performance`
...
- Arabic label: الفعاليات
- English label: Events
...
- Canonical candidate: `events` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `events` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Events workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-09/downtime-workspaces-browser.test.mjs, tests/build-10/build10-workspaces-contract.test.mjs, tests/build-10/cross-domain-scenarios.test.mjs, tests/build-10/fleet-telematics-geofences.test.mjs, tests/build-10/iot-offline-kiosk-export-contract.test.mjs, tests/build-10/iot-telemetry-health-commands.test.mjs, tests/build-11/commercial-platform-browser.test.mjs, tests/build-11/commercial-platform.test.mjs
...
- Events workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view, NOT VERIFIED, NOT VERIFIED.
