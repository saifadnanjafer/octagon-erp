---
page_id: "scenario_planner"
title_en: "Scenario Planner"
title_ar: "مخطط سيناريوهات الذكاء"
domain: "intelligence"
navigation_group: "intelligence_ai"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "scenario_planner"
parent_page: null
aliases: []
...
renderer_type: "view"
renderer_sources: ["views/scenario_planner.html"]
view_sources: ["views/scenario_planner.html"]
api_sources: []
...
- Page ID: `scenario_planner`
- English: Scenario Planner
- Arabic: مخطط سيناريوهات الذكاء
- Domain: `intelligence`; navigation group: `intelligence_ai`
- Route: switchPage('scenario_planner') / views/scenario_planner.html
- Page type: PAGE

# 2. Business Purpose

Scenario planner — manager/planning (risk: medium, phase: phase6e)
...
User opens this page to work with the Scenario Planner workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/scenario_planner.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/scenario_planner.html
...
- UI → renderer: views/scenario_planner.html
...
The intended output is the page’s named Scenario Planner record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: مخطط سيناريوهات الذكاء
- English label: Scenario Planner
...
- Canonical candidate: `scenario_planner` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `scenario_planner` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Scenario Planner workflow; the current 231/231 click audit is route activation evidence only.
...
- Scenario Planner workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
