---
page_id: "marketing_overview"
title_en: "Marketing Overview"
title_ar: "نظرة عامة على التسويق"
domain: "commercial"
navigation_group: "commercial_marketing"
kind: DASHBOARD
canonical_status: PRIMARY
canonical_home: "marketing_overview"
parent_page: null
aliases: []
...
- Page ID: `marketing_overview`
- English: Marketing Overview
- Arabic: نظرة عامة على التسويق
- Domain: `commercial`; navigation group: `commercial_marketing`
- Route: switchPage('marketing_overview') / views/marketing_overview.html
- Page type: DASHBOARD

# 2. Business Purpose

Marketing overview — build12/marketing (risk: medium, phase: build12)
...
User opens this page to work with the Marketing Overview workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/marketing_overview.html. Static source counts at baseline: 2 headings, 3 button tags, 2 input/select/textarea controls, 1 tables, 1 forms, and 2 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Marketing Overview record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: نظرة عامة على التسويق
- English label: Marketing Overview
...
The Marketing Overview workspace should give its governed users a page-specific way to complete the named business task: load scoped records, accept governed inputs where needed, execute authorized actions, persist the resulting state through the domain authority, and expose explicit loading/empty/denied/validation/server-error/success states. Exact fields and lifecycle transitions remain **NOT VERIFIED** where the baseline source does not define them.

# 17. Identified Gaps

- **PAGE-marketing_overview-GAP-004** (P1, PERMISSION) — The review inventory flags this commercial/SaaS page as lacking a matching entitlement mapping in the disposable review database.
- **PAGE-marketing_overview-GAP-005** (P2, TEST) — Documentation contradiction: NAVIGATION_FORENSIC_REPORT.json records app.js switchPage, while the module registry also contains this page. Reconcile renderer ownership after Product Recovery.
...
- Canonical candidate: `marketing_overview` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `marketing_overview` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Marketing Overview workflow; the current 231/231 click audit is route activation evidence only.
...
- Marketing Overview workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
