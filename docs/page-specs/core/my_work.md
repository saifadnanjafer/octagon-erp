...
- Route: switchPage('my_work') / views/my_work.html
- Page type: PAGE

# 2. Business Purpose

My Work — workshop/operations (risk: low, phase: internal-workshop-wave)

# 3. Primary Users

- Declared roles: workshop.user, workshop.manager, finance.user, finance.manager
- Viewer/operator/reviewer/manager/administrator split: NOT VERIFIED beyond the declared permission gate.

# 4. Primary User Goal

User opens this page to work with the My Work workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/my_work.html. Static source counts at baseline: 1 headings, 2 button tags, 5 input/select/textarea controls, 0 tables, 1 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/my_work.html
- Visible action inventory: Refresh; Clear
...
- UI → renderer: views/my_work.html
...
- UI label: Clear; action ID/API/domain handler/persistence: NOT VERIFIED by the baseline inventory.
...
- UI label: Clear; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.

# 8. Inputs

- Static controls: Refresh; Clear
...
The intended output is the page’s named My Work record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Client page gate: workshop.user, workshop.manager, finance.user, finance.manager
- Entitlement: none/global (not a commercial/SaaS-gated page)
...
- Canonical candidate: `my_work` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `my_work` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named My Work workflow; the current 231/231 click audit is route activation evidence only.
...
- My Work workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED.
