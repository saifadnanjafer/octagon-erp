---
page_id: "replenishment_proposals"
title_en: "Replenishment Proposals"
title_ar: "مقترحات التجديد"
domain: "ops"
navigation_group: "ops_warehouse_inbound"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "replenishment_proposals"
parent_page: null
aliases: []
...
renderer_sources: ["modules/build09-putaway-workspace.js"]
view_sources: []
api_sources: []
domain_sources: ["modules/build09-putaway-workspace.js","services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-09/build09-workspaces-contract.test.mjs","tests/build-09/build09r2-bespoke-contract.test.mjs","tests/build-09/cross-domain-lifecycles.test.mjs","tests/build-09/operational-32-page-matrix-chromium.test.mjs","tests/build-09/putaway-workspaces-browser.test.mjs"]
...
- Page ID: `replenishment_proposals`
- English: Replenishment Proposals
- Arabic: مقترحات التجديد
- Domain: `ops`; navigation group: `ops_warehouse_inbound`
- Route: switchPage('replenishment_proposals') / views/replenishment_proposals.html
- Page type: PAGE

# 2. Business Purpose

Replenishment proposals — workshop/inventory (risk: high, phase: build09)
...
User opens this page to work with the Replenishment Proposals workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/replenishment_proposals.html. Static source counts at baseline: 15 headings, 15 button tags, 0 input/select/textarea controls, 0 tables, 9 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: modules/build09-putaway-workspace.js
...
- UI → renderer: modules/build09-putaway-workspace.js
...
- UI label: ${esc(t('Scan source', 'مسح المصدر'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Scan destination', 'مسح الوجهة'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
- UI label: ${esc(t('Create rule', 'إنشاء القاعدة'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(row.productId)} ${esc(t('from', 'من'))} ${esc(row.sourceLocationId)} · ${esc(num(row.quantity))} ${badge(row.status, REC_TONE[row.status] ?? '')}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Accept recommendation', 'قبول التوصية'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Override', 'تجاوز'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Request putaway', 'طلب الإيداع'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Calculate now', 'احسب الآن'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(row.productId)} ${esc(t('proposed', 'المقترح'))} ${esc(num(row.proposedQuantity))} ${esc(t('of', 'من'))} ${esc(num(row.requestedQuantity))} ${badge(row.status, PROPOSAL_TONE[row.status] ?? '')}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Approve', 'اعتماد'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Replenishment Proposals record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.

# 10. Workflow Position

- Upstream: NOT VERIFIED from explicit workflow metadata.
- Downstream: `putaway_rules`, `putaway_task_queue`, `replenishment_rules`
...
- Arabic label: مقترحات التجديد
- English label: Replenishment Proposals
...
- Canonical candidate: `replenishment_proposals` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `replenishment_proposals` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Replenishment Proposals workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-09/build09-workspaces-contract.test.mjs, tests/build-09/build09r2-bespoke-contract.test.mjs, tests/build-09/cross-domain-lifecycles.test.mjs, tests/build-09/operational-32-page-matrix-chromium.test.mjs, tests/build-09/putaway-workspaces-browser.test.mjs
...
- Replenishment Proposals workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
