---
page_id: "warehouse_topology"
title_en: "Warehouse Topology"
title_ar: ""
domain: "ops"
navigation_group: "ops_inventory"
kind: PAGE
canonical_status: "PRIMARY"
canonical_home: "warehouse_topology"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
test_sources: ["tests/build-09/topology-workspaces-browser.test.mjs", "tests/build-09/operational-topology-chromium.test.mjs", "tests/build-09/build09-workspaces-contract.test.mjs"]
...
- Page ID: `warehouse_topology`; English: Warehouse Topology; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('warehouse_topology')`; page-specific renderer is `modules/build09-topology-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Define and inspect the warehouse hierarchy, zones, bins, capacity, and restrictions.
...
- Data loading: `hierarchy, zones, locations, capacity` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
...
- Renderer/domain source: `modules/build09-topology-workspace.js` → `topology.hierarchy/listZones/listLocations/capacityUtilization/createZone/createLocation`.
...
- The page renders scoped records from `hierarchy, zones, locations, capacity`, lifecycle/status badges, quantities and operational KPIs where applicable.
...
- Upstream: warehouse setup and company/warehouse context.
- Downstream: zone_bin_management, putaway_rules, putaway_task_queue.
...
- English label: Warehouse Topology; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-warehouse_topology-GAP-001** (P1, SCOPE) — hierarchy and location mutations must reject a warehouse outside the session company
- **PAGE-warehouse_topology-GAP-002** (P1, TEST) — a browser lifecycle must prove create/update/block/retire rather than route visibility only
...
- Canonical home: `warehouse_topology` unless a later owner decision explicitly changes the navigation contract.
...
- `tests/build-09/build09-workspaces-contract.test.mjs`
...
- `topology.hierarchy` → `listZones` → `listLocations` → `capacityUtilization` → `createZone` → `createLocation`
...
- Warehouse Topology page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
