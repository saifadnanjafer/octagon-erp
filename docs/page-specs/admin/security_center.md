---
page_id: "security_center"
title_en: "Security Center"
title_ar: "التدقيق والأمن"
domain: "admin"
navigation_group: "admin_org"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "security_center"
parent_page: null
aliases: []
roles: ["system.admin"]
...
renderer_type: "view"
renderer_sources: ["views/security_center.html"]
view_sources: ["views/security_center.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/phase02/browser-live-evidence.test.mjs"]
...
- Page ID: `security_center`
- English: Security Center
- Arabic: التدقيق والأمن
- Domain: `admin`; navigation group: `admin_org`
- Route: switchPage('security_center') / views/security_center.html
- Page type: PAGE

# 2. Business Purpose

Security center — admin/security (risk: critical, phase: phase6e)
...
User opens this page to work with the Security Center workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/security_center.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/security_center.html
...
- UI → renderer: views/security_center.html
...
The intended output is the page’s named Security Center record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: التدقيق والأمن
- English label: Security Center
...
- Canonical candidate: `security_center` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `security_center` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Security Center workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/phase02/browser-live-evidence.test.mjs
...
- Security Center workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
