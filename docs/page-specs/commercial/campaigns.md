...
renderer_sources: ["modules/jarvis-brain.js"]
view_sources: []
api_sources: ["/api/v1/build12/${resource}","/api/v1/action/${id}"]
domain_sources: ["modules/jarvis-brain.js","services/permissionService.js","modules/build12-workspaces.js"]
tables_or_entities: []
...
- Route: switchPage('campaigns') / views/campaigns.html
- Page type: PAGE

# 2. Business Purpose

Marketing campaigns — build12/campaigns (risk: high, phase: build12)
...
User opens this page to work with the Campaigns workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/campaigns.html. Static source counts at baseline: 2 headings, 3 button tags, 2 input/select/textarea controls, 1 tables, 1 forms, and 2 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: modules/jarvis-brain.js
...
- UI → renderer: modules/jarvis-brain.js
...
The intended output is the page’s named Campaigns record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: الحملات
- English label: Campaigns
...
- Canonical candidate: `campaigns` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `campaigns` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Campaigns workflow; the current 231/231 click audit is route activation evidence only.
...
- Campaigns workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
