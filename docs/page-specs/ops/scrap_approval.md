...
tables_or_entities: ["quality_dispositions", "quality_ncrs", "stock_moves"]
...
- Page ID: `scrap_approval`; English: Scrap Approval; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('scrap_approval')`; page-specific renderer is `modules/build09-quality-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Approve a scrap disposition, request canonical Inventory scrap, and acknowledge the verified result.
...
- Data loading: `quality-dispositions` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: quality:disposition_approve, quality:scrap_request_canonical, quality:scrap_acknowledge.
...
- Renderer/domain source: `modules/build09-quality-workspace.js` → `qualityOperations.listDispositions/approveDisposition/requestCanonicalScrap/acknowledgeCanonicalScrap`.
- Persisted entities/tables: quality_dispositions, quality_ncrs, stock_moves.
...
- `quality:disposition_approve`
- `quality:scrap_request_canonical`
- `quality:scrap_acknowledge`
...
- The page renders scoped records from `quality-dispositions`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into quality_dispositions, quality_ncrs, stock_moves and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: quality_hold_queue.
- Downstream: canonical inventory and traceability.
...
- English label: Scrap Approval; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-scrap_approval-GAP-001** (P1, PERMISSION) — requester cannot approve own disposition and this must be directly tested
- **PAGE-scrap_approval-GAP-002** (P1, WORKFLOW) — canonical acknowledgement must validate product, quantity, and destination before closure
...
- Canonical home: `scrap_approval` unless a later owner decision explicitly changes the navigation contract.
...
- `qualityOperations.listDispositions` → `approveDisposition` → `requestCanonicalScrap` → `acknowledgeCanonicalScrap`
...
- Scrap Approval page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
