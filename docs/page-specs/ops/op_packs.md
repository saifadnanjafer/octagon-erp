...
page_id: "op_packs"
title_en: "Op Packs"
title_ar: "باقات العمليات"
domain: "ops"
navigation_group: "ops_production"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "op_packs"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
renderer_type: "view"
renderer_sources: ["views/op_packs.html"]
...
- Page ID: `op_packs`
- English: Op Packs
- Arabic: باقات العمليات
- Domain: `ops`; navigation group: `ops_production`
- Route: switchPage('op_packs') / views/op_packs.html
- Page type: PAGE

# 2. Business Purpose

Operation packs — workshop/production (risk: medium, phase: core)
...
User opens this page to work with the Op Packs workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/op_packs.html. Static source counts at baseline: 1 headings, 1 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/op_packs.html
...
- UI → renderer: views/op_packs.html
...
- UI label: باقة جديدة; handler/action ID: addOpPack(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Op Packs record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: باقات العمليات
- English label: Op Packs
...
- **PAGE-op_packs-GAP-003** (P1, API) — No literal backend API path was verified in the inspected page-specific source; server capability remains NOT VERIFIED.
...
- Canonical candidate: `op_packs` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `op_packs` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Op Packs workflow; the current 231/231 click audit is route activation evidence only.
...
- Op Packs workspace presentation and its explicitly verified page-specific actions: addOpPack().
