---
page_id: "count_session"
title_en: "Count Session"
title_ar: ""
domain: "ops"
navigation_group: "ops_inventory"
kind: PAGE
canonical_status: "PRIMARY"
canonical_home: "count_session"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
- Page ID: `count_session`; English: Count Session; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('count_session')`; page-specific renderer is `modules/build09-count-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Start a scoped count, record observed quantities against theoretical lines, and submit the session for variance review.
...
- Primary actions exposed by this page: wms:count_session_start, wms:count_line_record, wms:count_submit.
...
- Renderer/domain source: `modules/build09-count-workspace.js` → `cycleCounting.startCountSession/recordCountLine/submitCount/listCountSessions`.
...
- `wms:count_session_start`
- `wms:count_line_record`
- `wms:count_submit`
...
- Upstream: cycle_count_plans.
- Downstream: variance_review, canonical inventory adjustment.
...
- English label: Count Session; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-count_session-GAP-001** (P1, SCOPE) — count session lines must remain warehouse/company scoped even when a product or location ID is supplied
- **PAGE-count_session-GAP-002** (P1, TEST) — prove blind-count behavior does not render theoretical quantity before submit
...
- Canonical home: `count_session` unless a later owner decision explicitly changes the navigation contract.
...
- `cycleCounting.startCountSession` → `recordCountLine` → `submitCount` → `listCountSessions`
...
- Count Session page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
