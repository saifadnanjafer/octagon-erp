...
view_sources: []
api_sources: []
domain_sources: ["modules/build09-pick-task-queue-workspace.js","services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-09/build09-workspaces-contract.test.mjs","tests/build-09/build09r2-bespoke-contract.test.mjs","tests/build-09/mobile-receiving-picking-browser.test.mjs","tests/build-09/operational-32-page-matrix-chromium.test.mjs","tests/build-09/pick-task-queue-denial-browser.test.mjs"]
...
- Page ID: `pick_task_queue`
- English: Pick Task Queue
- Arabic: طابور مهام الالتقاط
- Domain: `ops`; navigation group: `ops_warehouse_fulfillment`
- Route: switchPage('pick_task_queue') / views/pick_task_queue.html
- Page type: QUEUE

# 2. Business Purpose

Pick task queue — workshop/inventory (risk: medium, phase: build09)
...
User opens this page to work with the Pick Task Queue workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/pick_task_queue.html. Static source counts at baseline: 2 headings, 3 button tags, 4 input/select/textarea controls, 0 tables, 1 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: modules/build09-pick-task-queue-workspace.js
...
- UI → renderer: modules/build09-pick-task-queue-workspace.js
...
- UI label: ${esc(x.reference || x.id)} ${badge(x.status, tone[x.status] || '')} ${esc(t('Qty','الكمية'))}: ${esc(num(x.quantity,2))} · ${esc(t('Picker','الملتقط'))}: ${esc(x.assignedTo || '—')}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Pick Task Queue record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.

# 10. Workflow Position

- Upstream: NOT VERIFIED from explicit workflow metadata.
- Downstream: `mobile_picking`
...
- Arabic label: طابور مهام الالتقاط
- English label: Pick Task Queue
...
- Canonical candidate: `pick_task_queue` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `pick_task_queue` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Pick Task Queue workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-09/build09-workspaces-contract.test.mjs, tests/build-09/build09r2-bespoke-contract.test.mjs, tests/build-09/mobile-receiving-picking-browser.test.mjs, tests/build-09/operational-32-page-matrix-chromium.test.mjs, tests/build-09/pick-task-queue-denial-browser.test.mjs
...
- Pick Task Queue workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
