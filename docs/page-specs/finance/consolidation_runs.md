...
renderer_type: "view"
renderer_sources: ["views/consolidation_runs.html"]
...
- Page ID: `consolidation_runs`
- English: Consolidation Runs
- Arabic: · عمليات التوحيد
- Domain: `finance`; navigation group: `finance_consolidation`
- Route: switchPage('consolidation_runs') / views/consolidation_runs.html
- Page type: TRANSACTION

# 2. Business Purpose

Consolidation runs — finance/consolidation (risk: critical, phase: build08)
...
User opens this page to work with the Consolidation Runs workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/consolidation_runs.html. Static source counts at baseline: 1 headings, 0 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/consolidation_runs.html
...
- UI → renderer: views/consolidation_runs.html
...
The intended output is the page’s named Consolidation Runs record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: · عمليات التوحيد
- English label: Consolidation Runs
...
- Canonical candidate: `consolidation_runs` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `consolidation_runs` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Consolidation Runs workflow; the current 231/231 click audit is route activation evidence only.
...
- Consolidation Runs workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
