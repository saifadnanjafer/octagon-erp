---
page_id: "shopfloor_terminal"
title_en: "Shopfloor Terminal"
title_ar: ""
domain: "ops"
navigation_group: "ops_inventory"
kind: PAGE
canonical_status: "PRIMARY"
canonical_home: "shopfloor_terminal"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
domain_sources: ["modules/build09-shopfloor-workspace.js", "platform/wms/index.mjs", "platform/manufacturing/downtime-performance.mjs"]
tables_or_entities: ["shopfloor_sessions", "shopfloor_operations", "shopfloor_outputs"]
test_sources: ["tests/build-09/shopfloor-workspaces-browser.test.mjs", "tests/build-09/production-material-workspaces-browser.test.mjs", "tests/build-09/operational-production-quality-chromium.test.mjs"]
...
- Page ID: `shopfloor_terminal`; English: Shopfloor Terminal; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('shopfloor_terminal')`; page-specific renderer is `modules/build09-shopfloor-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Run a work-center operation, assign an operator, record output, and hand off or acknowledge the canonical production result.
...
- Renderer: `modules/build09-shopfloor-workspace.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `shopfloor-sessions, shopfloor-timeline, shopfloor-board` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: shopfloor:operation_output, shopfloor:operation_acknowledge, shopfloor:operator_assign, shopfloor:operation_handoff.
...
- Renderer/domain source: `modules/build09-shopfloor-workspace.js` → `shopfloor list sessions/status/timeline plus operation output/acknowledgement/handoff handlers`.
- Persisted entities/tables: shopfloor_sessions, shopfloor_operations, shopfloor_outputs.
...
- `shopfloor:operation_output`
- `shopfloor:operation_acknowledge`
- `shopfloor:operator_assign`
- `shopfloor:operation_handoff`
...
- The page renders scoped records from `shopfloor-sessions, shopfloor-timeline, shopfloor-board`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into shopfloor_sessions, shopfloor_operations, shopfloor_outputs and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: workcenter_queue, production_material_requests.
- Downstream: production_receipt, downtime_board, quality_hold_queue.
...
- English label: Shopfloor Terminal; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-shopfloor_terminal-GAP-001** (P1, WORKFLOW) — operation output versus canonical production receipt boundary needs end-to-end proof
- **PAGE-shopfloor_terminal-GAP-002** (P1, PERMISSION) — operator assignment/handoff must be tested against session scope and maker-checker rules
...
- Canonical home: `shopfloor_terminal` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-shopfloor-workspace.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/shopfloor-workspaces-browser.test.mjs`
- `tests/build-09/production-material-workspaces-browser.test.mjs`
- `tests/build-09/operational-production-quality-chromium.test.mjs`
...
- `shopfloor list sessions` → `status` → `timeline plus operation output` → `acknowledgement` → `handoff handlers`
...
- Shopfloor Terminal page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
