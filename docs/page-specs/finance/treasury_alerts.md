...
renderer_type: "view"
renderer_sources: ["views/treasury_alerts.html"]
...
- Page ID: `treasury_alerts`
- English: Treasury Alerts
- Arabic: · تنبيهات الخزينة
- Domain: `finance`; navigation group: `finance_treasury`
- Route: switchPage('treasury_alerts') / views/treasury_alerts.html
- Page type: PAGE

# 2. Business Purpose

Treasury alerts — finance/treasury (risk: high, phase: build08)
...
User opens this page to work with the Treasury Alerts workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/treasury_alerts.html. Static source counts at baseline: 1 headings, 0 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/treasury_alerts.html
...
- UI → renderer: views/treasury_alerts.html
...
The intended output is the page’s named Treasury Alerts record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: · تنبيهات الخزينة
- English label: Treasury Alerts
...
- Canonical candidate: `treasury_alerts` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `treasury_alerts` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Treasury Alerts workflow; the current 231/231 click audit is route activation evidence only.
...
- Treasury Alerts workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
