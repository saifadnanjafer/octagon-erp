...
domain_sources: ["modules/build09-wave-workspace.js", "platform/wms/index.mjs", "platform/wms/waves.mjs"]
tables_or_entities: ["wms_pick_waves", "wms_pick_wave_tasks", "wms_pick_tasks_v2", "wms_warehouse_tasks"]
test_sources: ["tests/build-09/wave-workspaces-browser.test.mjs", "tests/build-09/operational-32-page-matrix-chromium.test.mjs", "tests/build-09/cross-domain-lifecycles.test.mjs"]
...
- Page ID: `wave_execution`; English: Wave Execution; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('wave_execution')`; page-specific renderer is `modules/build09-wave-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Release reviewed waves, monitor task progress and staging, handle exceptions, and close the wave when work is complete.
...
- Renderer: `modules/build09-wave-workspace.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `waves` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:wave_release, wms:wave_refresh_progress, wms:wave_cancel, wms:wave_complete.
...
- Renderer/domain source: `modules/build09-wave-workspace.js` → `waves.releaseWave/refreshWaveProgress/cancelWave/completeWave/listWaves`.
- Persisted entities/tables: wms_pick_waves, wms_pick_wave_tasks, wms_pick_tasks_v2, wms_warehouse_tasks.
...
- The page renders scoped records from `waves`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_pick_waves, wms_pick_wave_tasks, wms_pick_tasks_v2, wms_warehouse_tasks and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: wave_planning.
- Downstream: mobile_picking, staging_board, canonical inventory.
...
- English label: Wave Execution; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-wave_execution-GAP-001** (P1, WORKFLOW) — release-to-completion and exception refresh must be proven with task state changes
- **PAGE-wave_execution-GAP-002** (P1, ERROR_STATE) — cancellation and blocked waves need explicit browser-visible reason handling
...
- Canonical home: `wave_execution` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-wave-workspace.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/wave-workspaces-browser.test.mjs`
...
- `waves.releaseWave` → `refreshWaveProgress` → `cancelWave` → `completeWave` → `listWaves`
...
- Wave Execution page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
