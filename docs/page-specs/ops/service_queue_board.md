...
- Page ID: `service_queue_board`
- English: Service Queue Board
- Arabic: لوحة طابور الخدمة
- Domain: `ops`; navigation group: `ops_boards`
- Route: switchPage('service_queue_board') / views/service_queue_board.html
- Page type: QUEUE

# 2. Business Purpose

Service queue board — boards/service (risk: low, phase: build10)
...
User opens this page to work with the Service Queue Board workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/service_queue_board.html. Static source counts at baseline: 7 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Service Queue Board record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: لوحة طابور الخدمة
- English label: Service Queue Board
...
- Canonical candidate: `service_queue_board` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `service_queue_board` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Service Queue Board workflow; the current 231/231 click audit is route activation evidence only.
...
- Service Queue Board workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
