...
tables_or_entities: ["shopfloor_sessions", "work_orders", "work_centers"]
test_sources: ["tests/build-09/shopfloor-workspaces-browser.test.mjs", "tests/build-09/operational-production-quality-chromium.test.mjs"]
...
- Page ID: `workcenter_queue`; English: Work-center Queue; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('workcenter_queue')`; page-specific renderer is `modules/build09-shopfloor-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Prioritize ready, running, blocked, and quality-held work-center sessions and open the terminal for execution.
...
- Data loading: `shopfloor-sessions, shopfloor-board` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: shopfloor:operator_assign, shopfloor:operation_handoff.
...
- Renderer/domain source: `modules/build09-shopfloor-workspace.js` → `shopfloor.listShopfloorSessions/shopfloorStatusBoard and operation assignment/handoff handlers`.
- Persisted entities/tables: shopfloor_sessions, work_orders, work_centers.
...
- The page renders scoped records from `shopfloor-sessions, shopfloor-board`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into shopfloor_sessions, work_orders, work_centers and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: production orders and material availability.
- Downstream: shopfloor_terminal, downtime_board.
...
- English label: Work-center Queue; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-workcenter_queue-GAP-001** (P1, SCOPE) — queue state must be warehouse/company scoped
- **PAGE-workcenter_queue-GAP-002** (P1, ERROR_STATE) — blocked and quality-held sessions need actionable reasons, not just status badges
...
- Canonical home: `workcenter_queue` unless a later owner decision explicitly changes the navigation contract.
...
- `shopfloor.listShopfloorSessions` → `shopfloorStatusBoard and operation assignment` → `handoff handlers`
...
- Work-center Queue page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
