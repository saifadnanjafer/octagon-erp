...
renderer_type: "view"
renderer_sources: ["views/expenses.html"]
...
- Route: switchPage('expenses') / views/expenses.html
- Page type: PAGE

# 2. Business Purpose

Expenses — finance (risk: high, phase: core)
...
User opens this page to work with the Expenses workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/expenses.html. Static source counts at baseline: 3 headings, 3 button tags, 7 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/expenses.html
...
- UI → renderer: views/expenses.html
...
- UI label: تسجيل المصروف; handler/action ID: addExpenseFromForm(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Expenses record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: المصروفات
- English label: Expenses
...
- Canonical candidate: `expenses` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `expenses` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Expenses workflow; the current 231/231 click audit is route activation evidence only.
...
- Expenses workspace presentation and its explicitly verified page-specific actions: addFinanceDemoData(, addExpenseFromForm(), switchPage(.
