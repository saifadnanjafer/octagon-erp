---
page_id: "integration_hub"
title_en: "Integration Hub"
title_ar: "مركز التكامل"
domain: "admin"
navigation_group: "admin_org"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "integration_hub"
parent_page: null
aliases: []
roles: ["system.admin"]
...
renderer_type: "view"
renderer_sources: ["views/integration_hub.html"]
view_sources: ["views/integration_hub.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-04/integration-hub-rendering.test.mjs"]
...
- Page ID: `integration_hub`
- English: Integration Hub
- Arabic: مركز التكامل
- Domain: `admin`; navigation group: `admin_org`
- Route: switchPage('integration_hub') / views/integration_hub.html
- Page type: PAGE

# 2. Business Purpose

Integration hub — admin/integration (risk: critical, phase: phase6e)

# 3. Primary Users

- Declared roles: system.admin
- Viewer/operator/reviewer/manager/administrator split: NOT VERIFIED beyond the declared permission gate.

# 4. Primary User Goal

User opens this page to work with the Integration Hub workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/integration_hub.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/integration_hub.html
...
- UI → renderer: views/integration_hub.html
...
The intended output is the page’s named Integration Hub record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: مركز التكامل
- English label: Integration Hub
...
- Canonical candidate: `integration_hub` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `integration_hub` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Integration Hub workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-04/integration-hub-rendering.test.mjs
...
- Integration Hub workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
