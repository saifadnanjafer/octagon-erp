...
renderer_type: "view"
renderer_sources: ["views/supplier_portal.html"]
...
- Page ID: `supplier_portal`
- English: Supplier Portal
- Arabic: بوابة الموردين
- Domain: `resources`; navigation group: `resources_supply`
- Route: switchPage('supplier_portal') / views/supplier_portal.html
- Page type: PAGE

# 2. Business Purpose

Supplier portal — procurement/finance (risk: high, phase: phase6e)
...
User opens this page to work with the Supplier Portal workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/supplier_portal.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/supplier_portal.html
...
- UI → renderer: views/supplier_portal.html
...
The intended output is the page’s named Supplier Portal record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: بوابة الموردين
- English label: Supplier Portal
...
- Canonical candidate: `supplier_portal` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `supplier_portal` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Supplier Portal workflow; the current 231/231 click audit is route activation evidence only.
...
- Supplier Portal workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
