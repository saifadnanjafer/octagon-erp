---
page_id: "ai_context_sources"
title_en: "AI Context Sources"
title_ar: "مصادر سياق الذكاء الاصطناعي"
domain: "intelligence"
navigation_group: "intelligence_governance"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "ai_context_sources"
parent_page: null
aliases: []
...
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-12/build12-acceptance-contract.test.mjs","tests/build-12/build12-browser.test.mjs"]
...
- Page ID: `ai_context_sources`
- English: AI Context Sources
- Arabic: مصادر سياق الذكاء الاصطناعي
- Domain: `intelligence`; navigation group: `intelligence_governance`
- Route: switchPage('ai_context_sources') / views/ai_context_sources.html
- Page type: PAGE

# 2. Business Purpose

AI context sources — build12/ai-context (risk: high, phase: build12)
...
User opens this page to work with the AI Context Sources workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/ai_context_sources.html. Static source counts at baseline: 2 headings, 3 button tags, 2 input/select/textarea controls, 1 tables, 1 forms, and 2 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named AI Context Sources record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: مصادر سياق الذكاء الاصطناعي
- English label: AI Context Sources
...
The AI Context Sources workspace should give its governed users a page-specific way to complete the named business task: load scoped records, accept governed inputs where needed, execute authorized actions, persist the resulting state through the domain authority, and expose explicit loading/empty/denied/validation/server-error/success states. Exact fields and lifecycle transitions remain **NOT VERIFIED** where the baseline source does not define them.

# 17. Identified Gaps

- **PAGE-ai_context_sources-GAP-004** (P1, PERMISSION) — The review inventory flags this commercial/SaaS page as lacking a matching entitlement mapping in the disposable review database.
- **PAGE-ai_context_sources-GAP-005** (P2, TEST) — Documentation contradiction: NAVIGATION_FORENSIC_REPORT.json records app.js switchPage, while the module registry also contains this page. Reconcile renderer ownership after Product Recovery.
...
- Canonical candidate: `ai_context_sources` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `ai_context_sources` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named AI Context Sources workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-12/build12-acceptance-contract.test.mjs, tests/build-12/build12-browser.test.mjs
...
- AI Context Sources workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
