...
page_id: "kiosk"
title_en: "Kiosk"
title_ar: "روح النظام (كيوسك)"
domain: "ops"
navigation_group: "ops_kiosks"
kind: KIOSK
canonical_status: PRIMARY
canonical_home: "kiosk"
parent_page: null
aliases: []
...
renderer_sources: ["views/kiosk.html","modules/build10-workspaces.js"]
...
- Page ID: `kiosk`
- English: Kiosk
- Arabic: روح النظام (كيوسك)
- Domain: `ops`; navigation group: `ops_kiosks`
- Route: switchPage('kiosk') / views/kiosk.html
- Page type: KIOSK

# 2. Business Purpose

Kiosk Terminal — public/display (risk: low, phase: phase6h)
...
User opens this page to work with the Kiosk workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/kiosk.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/kiosk.html
...
The intended output is the page’s named Kiosk record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Canonical candidate: `kiosk` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `kiosk` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Kiosk workflow; the current 231/231 click audit is route activation evidence only.
...
- Kiosk workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
