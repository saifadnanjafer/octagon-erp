...
tables_or_entities: ["quality_rework_routes", "quality_rework_operations"]
...
- Page ID: `rework_workspace`; English: Rework Workspace; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('rework_workspace')`; page-specific renderer is `modules/build09-quality-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Start an approved rework route, monitor its operations, complete it, and surface the required retest proposal.
...
- Data loading: `rework-routes` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: quality:rework_start, quality:rework_complete.
...
- Renderer/domain source: `modules/build09-quality-workspace.js` → `qualityOperations.listReworkRoutes/startRework/completeRework`.
- Persisted entities/tables: quality_rework_routes, quality_rework_operations.
...
- `quality:rework_start`
- `quality:rework_complete`
...
- The page renders scoped records from `rework-routes`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into quality_rework_routes, quality_rework_operations and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: quality_hold_queue.
- Downstream: quality_hold_queue retest, shopfloor terminal.
...
- English label: Rework Workspace; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-rework_workspace-GAP-001** (P1, WORKFLOW) — completion creates a retest proposal rather than passing quality automatically
- **PAGE-rework_workspace-GAP-002** (P1, TEST) — prove route status transitions and retest requirement with a non-empty fixture
...
- Canonical home: `rework_workspace` unless a later owner decision explicitly changes the navigation contract.
...
- `qualityOperations.listReworkRoutes` → `startRework` → `completeRework`
...
- Rework Workspace page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
