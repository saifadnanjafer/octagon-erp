...
renderer_type: "view"
renderer_sources: ["views/help_manual.html"]
view_sources: ["views/help_manual.html"]
api_sources: []
...
- Route: switchPage('help_manual') / views/help_manual.html
- Page type: PAGE

# 2. Business Purpose

Help manual — employee_self_service (risk: low, phase: phase6h)
...
User opens this page to work with the Help Manual workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/help_manual.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/help_manual.html
...
- UI → renderer: views/help_manual.html
...
The intended output is the page’s named Help Manual record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: دليل الاستخدام
- English label: Help Manual
...
- Canonical candidate: `help_manual` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `help_manual` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Help Manual workflow; the current 231/231 click audit is route activation evidence only.
...
- Help Manual workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
