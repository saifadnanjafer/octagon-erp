...
renderer_type: "view"
renderer_sources: ["views/consolidation_groups.html"]
view_sources: ["views/consolidation_groups.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-08/build08-page-contract.test.mjs","tests/build-08/platform-governance-contract.test.mjs"]
...
- Page ID: `consolidation_groups`
- English: Consolidation Groups
- Arabic: · مجموعات التوحيد
- Domain: `finance`; navigation group: `finance_consolidation`
- Route: switchPage('consolidation_groups') / views/consolidation_groups.html
- Page type: PAGE

# 2. Business Purpose

Consolidation groups — finance/consolidation (risk: critical, phase: build08)
...
User opens this page to work with the Consolidation Groups workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/consolidation_groups.html. Static source counts at baseline: 1 headings, 0 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/consolidation_groups.html
...
- UI → renderer: views/consolidation_groups.html
...
The intended output is the page’s named Consolidation Groups record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: · مجموعات التوحيد
- English label: Consolidation Groups
...
- Canonical candidate: `consolidation_groups` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `consolidation_groups` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Consolidation Groups workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-08/build08-page-contract.test.mjs, tests/build-08/platform-governance-contract.test.mjs
...
- Consolidation Groups workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
