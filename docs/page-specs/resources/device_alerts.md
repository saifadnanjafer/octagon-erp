---
page_id: "device_alerts"
title_en: "Device Alerts"
title_ar: "تنبيهات الأجهزة"
domain: "resources"
navigation_group: "resources_devices"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "device_alerts"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
renderer_sources: ["modules/build10/renderers/telemetry.js"]
view_sources: []
...
domain_sources: ["modules/build10/renderers/telemetry.js","services/permissionService.js","modules/build10-workspaces.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-10/build10-workspaces-contract.test.mjs","tests/build-10/iot-telemetry-health-commands.test.mjs","tests/workshop/command-center-domain.test.mjs","tests/workshop/drilldown-domain.test.mjs"]
...
- Page ID: `device_alerts`
- English: Device Alerts
- Arabic: تنبيهات الأجهزة
- Domain: `resources`; navigation group: `resources_devices`
- Route: switchPage('device_alerts') / views/device_alerts.html
- Page type: PAGE

# 2. Business Purpose

Device alerts — iot/alerts (risk: high, phase: build10)
...
User opens this page to work with the Device Alerts workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/device_alerts.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: modules/build10/renderers/telemetry.js
...
- UI → renderer: modules/build10/renderers/telemetry.js
...
The intended output is the page’s named Device Alerts record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: تنبيهات الأجهزة
- English label: Device Alerts
...
- Canonical candidate: `device_alerts` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `device_alerts` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Device Alerts workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-10/build10-workspaces-contract.test.mjs, tests/build-10/iot-telemetry-health-commands.test.mjs, tests/workshop/command-center-domain.test.mjs, tests/workshop/drilldown-domain.test.mjs
...
- Device Alerts workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
