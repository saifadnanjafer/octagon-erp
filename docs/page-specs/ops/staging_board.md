---
page_id: "staging_board"
title_en: "Staging Board"
title_ar: ""
domain: "ops"
navigation_group: "ops_inventory"
kind: DASHBOARD
canonical_status: "PRIMARY"
canonical_home: "staging_board"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
tables_or_entities: ["wms_staging_allocations", "wms_dock_appointments_v2", "wms_location_profiles"]
test_sources: ["tests/build-09/dock-workspaces-browser.test.mjs", "tests/build-09/dock-crossdock-domain.test.mjs", "tests/build-09/operational-32-page-matrix-chromium.test.mjs"]
...
- Page ID: `staging_board`; English: Staging Board; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('staging_board')`; page-specific renderer is `modules/build09-dock-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Allocate and release staging capacity for dock, wave, receiving, or cross-dock work.
...
- Data loading: `staging-allocations, locations` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:staging_allocate, wms:staging_release.
...
- Renderer/domain source: `modules/build09-dock-workspace.js` → `docks.allocateStaging/releaseStaging/listStagingAllocations`.
- Persisted entities/tables: wms_staging_allocations, wms_dock_appointments_v2, wms_location_profiles.
...
- The page renders scoped records from `staging-allocations, locations`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_staging_allocations, wms_dock_appointments_v2, wms_location_profiles and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: dock_checkin, wave_execution, mobile_receiving.
- Downstream: crossdock_workspace, canonical movements.
...
- English label: Staging Board; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-staging_board-GAP-001** (P1, SCOPE) — allocation must prevent over-capacity and cross-warehouse location selection
- **PAGE-staging_board-GAP-002** (P1, TEST) — prove release does not remove the underlying appointment or wave record
...
- Canonical home: `staging_board` unless a later owner decision explicitly changes the navigation contract.
...
- `docks.allocateStaging` → `releaseStaging` → `listStagingAllocations`
...
- Staging Board page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
