---
page_id: "maintenance_triggers"
title_en: "Maintenance Triggers"
title_ar: "محفزات الصيانة"
domain: "resources"
navigation_group: "resources_fleet"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "maintenance_triggers"
parent_page: null
aliases: []
...
- Page ID: `maintenance_triggers`
- English: Maintenance Triggers
- Arabic: محفزات الصيانة
- Domain: `resources`; navigation group: `resources_fleet`
- Route: switchPage('maintenance_triggers') / views/maintenance_triggers.html
- Page type: PAGE

# 2. Business Purpose

Maintenance triggers — fleet/maintenance (risk: high, phase: build10)
...
User opens this page to work with the Maintenance Triggers workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/maintenance_triggers.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Maintenance Triggers record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: محفزات الصيانة
- English label: Maintenance Triggers
...
- Canonical candidate: `maintenance_triggers` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `maintenance_triggers` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Maintenance Triggers workflow; the current 231/231 click audit is route activation evidence only.
...
- Maintenance Triggers workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
