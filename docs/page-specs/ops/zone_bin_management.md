---
page_id: "zone_bin_management"
title_en: "Zone and Bin Management"
title_ar: ""
domain: "ops"
navigation_group: "ops_inventory"
kind: PAGE
canonical_status: "PRIMARY"
canonical_home: "zone_bin_management"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
renderer_sources: ["modules/build09-topology-workspace.js"]
view_sources: []
api_sources: ["platform/api/build09.mjs"]
domain_sources: ["modules/build09-topology-workspace.js", "platform/wms/index.mjs", "platform/wms/topology.mjs"]
tables_or_entities: ["wms_zones", "wms_location_profiles", "stock_locations"]
test_sources: ["tests/build-09/topology-workspaces-browser.test.mjs", "tests/build-09/operational-topology-chromium.test.mjs"]
...
- Page ID: `zone_bin_management`; English: Zone and Bin Management; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('zone_bin_management')`; page-specific renderer is `modules/build09-topology-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Maintain operational zones and bin profiles used by receiving, putaway, picking, and staging.
...
- Renderer: `modules/build09-topology-workspace.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `zones, locations, capacity` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:zone_create, wms:location_create.
...
- Renderer/domain source: `modules/build09-topology-workspace.js` → `topology.createZone/createLocation/updateLocation/setLocationCapacity/setLocationRestrictions/generateLocationBarcode/retireLocation`.
- Persisted entities/tables: wms_zones, wms_location_profiles, stock_locations.
...
- The page renders scoped records from `zones, locations, capacity`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_zones, wms_location_profiles, stock_locations and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: warehouse_topology.
- Downstream: putaway_rules, mobile_receiving, mobile_picking, staging_board.
...
- English label: Zone and Bin Management; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-zone_bin_management-GAP-001** (P1, ACTION) — the renderer exposes creation-focused controls while the domain also supports edit, capacity, restriction, barcode, and retirement operations
- **PAGE-zone_bin_management-GAP-002** (P1, WORKFLOW) — ownership of those controls is incomplete
- **PAGE-zone_bin_management-GAP-003** (P1, TEST) — prove retired/blocked bins disappear from governed selectors
...
- Canonical home: `zone_bin_management` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-topology-workspace.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/topology-workspaces-browser.test.mjs`
- `tests/build-09/operational-topology-chromium.test.mjs`
...
- `topology.createZone` → `createLocation` → `updateLocation` → `setLocationCapacity` → `setLocationRestrictions` → `generateLocationBarcode` → `retireLocation`
...
- Zone and Bin Management page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
