...
page_id: "telegram"
title_en: "Telegram"
title_ar: "بوابة Telegram"
domain: "intelligence"
navigation_group: "intelligence_core"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "telegram"
parent_page: null
aliases: []
...
renderer_type: "view"
renderer_sources: ["views/telegram.html"]
view_sources: ["views/telegram.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/unit/telegram-bot.test.mjs"]
...
- Page ID: `telegram`
- English: Telegram
- Arabic: بوابة Telegram
- Domain: `intelligence`; navigation group: `intelligence_core`
- Route: switchPage('telegram') / views/telegram.html
- Page type: PAGE

# 2. Business Purpose

Telegram connector — ai/customer_comms (risk: high, phase: core)
...
User opens this page to work with the Telegram workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/telegram.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/telegram.html
...
- UI → renderer: views/telegram.html
...
The intended output is the page’s named Telegram record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: بوابة Telegram
- English label: Telegram
...
- Canonical candidate: `telegram` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `telegram` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Telegram workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/unit/telegram-bot.test.mjs
...
- Telegram workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
