...
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-12/build12-acceptance-contract.test.mjs","tests/build-12/build12-browser.test.mjs","tests/navigation/workshop-pack-setup-to-readiness.test.mjs"]
...
- Page ID: `workshop_pack_setup`
- English: Workshop Pack Setup
- Arabic: إعداد حزمة الورشة
- Domain: `commercial`; navigation group: `commercial_verticals`
- Route: switchPage('workshop_pack_setup') / views/workshop_pack_setup.html
- Page type: SETUP

# 2. Business Purpose

Workshop pack setup — build12/pack-installations (risk: high, phase: build12)
...
User opens this page to work with the Workshop Pack Setup workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/workshop_pack_setup.html. Static source counts at baseline: 2 headings, 3 button tags, 2 input/select/textarea controls, 1 tables, 1 forms, and 2 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
- Required fixture: al-warsha-pack
...
The intended output is the page’s named Workshop Pack Setup record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: إعداد حزمة الورشة
- English label: Workshop Pack Setup
...
The Workshop Pack Setup workspace should give its governed users a page-specific way to complete the named business task: load scoped records, accept governed inputs where needed, execute authorized actions, persist the resulting state through the domain authority, and expose explicit loading/empty/denied/validation/server-error/success states. Exact fields and lifecycle transitions remain **NOT VERIFIED** where the baseline source does not define them.

# 17. Identified Gaps

- **PAGE-workshop_pack_setup-GAP-004** (P1, PERMISSION) — The review inventory flags this commercial/SaaS page as lacking a matching entitlement mapping in the disposable review database.
- **PAGE-workshop_pack_setup-GAP-005** (P2, TEST) — Documentation contradiction: NAVIGATION_FORENSIC_REPORT.json records app.js switchPage, while the module registry also contains this page. Reconcile renderer ownership after Product Recovery.
...
- Canonical candidate: `workshop_pack_setup` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `workshop_pack_setup` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Workshop Pack Setup workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-12/build12-acceptance-contract.test.mjs, tests/build-12/build12-browser.test.mjs, tests/navigation/workshop-pack-setup-to-readiness.test.mjs
...
- Workshop Pack Setup workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
