...
(function actionForms(root) {
  ...
  const registry = {
    ...
    'quality:checkpoint_open': { title: t('Open Quality Checkpoint', 'فتح نقطة فحص جودة'), fields: [select('source_type', t('Source', 'المصدر'), [opt('shopfloor_session', 'Shop-floor session', 'جلسة أرضية المصنع'), opt('receiving_session', 'Receiving session', 'جلسة استلام'), opt('production_order', 'Production order', 'أمر إنتاج'), opt('work_order', 'Work order', 'أمر عمل')], { required: true }), text('source_id', t('Source ID', 'معرّف المصدر'), { required: true }), lookup('product_id', t('Product', 'المنتج'), 'products', { required: true }), select('checkpoint_type', t('Checkpoint type', 'نوع نقطة الفحص'), [opt('incoming', 'Incoming', 'وارد'), opt('in_process', 'In-process', 'أثناء العملية'), opt('final', 'Final', 'نهائي')], { required: true }), text('inspection_id', t('Inspection ID', 'معرّف الفحص'), { required: true })] },
    ...
    host.querySelectorAll('[data-lookup-resource]').forEach((wrapper) => {
      ...
      const fill = async (query, isPreload) => {
        ...
        const head = rows.length
          ? (rtl() ? '— اختر —' : '— Select —')
          : (query ? (rtl() ? 'لا توجد نتائج' : 'No match') : (rtl() ? 'لا توجد عناصر' : 'None available'));
        ...
        resultSelect.innerHTML = `<option value="">${head}</option>` + rows.map((row) => `<option value="${escapeHtml(row.id)}">${escapeHtml(row.label)}</option>`).join('');
        if (prevValue) {
      ...
      // Populate the first page up front so a required lookup field is not a
      // dead end for an operator who cannot recite its values from memory.
      fill('', true);
    });
  }
