...
renderer_type: "view"
renderer_sources: ["views/projects.html"]
view_sources: ["views/projects.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/checkpoint-d-e/projects_lifecycle.test.mjs","tests/checkpoint-d-e/shell_dispatcher.test.mjs","tests/checkpoint-f/canonical_authority_coverage.test.mjs","tests/checkpoint-f/cross_domain_record_integrity.test.mjs","tests/checkpoint-g/canonical_cutover_controller.test.mjs","tests/checkpoint-g/failure_injection_complete.test.mjs","tests/checkpoint-h/http_legacy_writer_refusal.test.mjs","tests/migration/health-only-mode.test.mjs"]
...
- Route: switchPage('projects') / views/projects.html
- Page type: PAGE

# 2. Business Purpose

Project Hub — workshop/operations (risk: medium, phase: phase6h)
...
User opens this page to work with the Projects workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/projects.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/projects.html
...
- UI → renderer: views/projects.html
...
The intended output is the page’s named Projects record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: المشاريع
- English label: Projects
...
- Canonical candidate: `projects` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `projects` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Projects workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/checkpoint-d-e/projects_lifecycle.test.mjs, tests/checkpoint-d-e/shell_dispatcher.test.mjs, tests/checkpoint-f/canonical_authority_coverage.test.mjs, tests/checkpoint-f/cross_domain_record_integrity.test.mjs, tests/checkpoint-g/canonical_cutover_controller.test.mjs, tests/checkpoint-g/failure_injection_complete.test.mjs, tests/checkpoint-h/http_legacy_writer_refusal.test.mjs, tests/migration/health-only-mode.test.mjs
...
- Projects workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
