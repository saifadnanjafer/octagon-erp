...
renderer_type: "view"
renderer_sources: ["views/cashbox.html"]
view_sources: ["views/cashbox.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/checkpoint-c/canonical_pos_ui.test.mjs","tests/checkpoint-c/pos_atomic_lifecycle.test.mjs","tests/phase03/finance-closure-audit.test.mjs","tests/phase03/finance-wave-d.test.mjs","tests/phase04/canonical_pos.test.mjs"]
...
- Route: switchPage('cashbox') / views/cashbox.html
- Page type: PAGE

# 2. Business Purpose

Cashbox — finance (risk: high, phase: core)
...
User opens this page to work with the Cashbox workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/cashbox.html. Static source counts at baseline: 1 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/cashbox.html
...
- UI → renderer: views/cashbox.html
...
- UI label: إضافة حركة نقدية; handler/action ID: openCashboxTransactionModal(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: إضافة بيانات تجريبية; handler/action ID: addFinanceDemoData(; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ›; handler/action ID: shiftCashboxDate(-1); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: اليوم; handler/action ID: setCashboxToday(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ‹; handler/action ID: shiftCashboxDate(1); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Cashbox record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: قاصة الورشة
- English label: Cashbox
...
- Canonical candidate: `cashbox` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `cashbox` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Cashbox workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/checkpoint-c/canonical_pos_ui.test.mjs, tests/checkpoint-c/pos_atomic_lifecycle.test.mjs, tests/phase03/finance-closure-audit.test.mjs, tests/phase03/finance-wave-d.test.mjs, tests/phase04/canonical_pos.test.mjs
...
- Cashbox workspace presentation and its explicitly verified page-specific actions: openCashboxTransactionModal(), addFinanceDemoData(, shiftCashboxDate(-1), setCashboxToday(), shiftCashboxDate(1).
