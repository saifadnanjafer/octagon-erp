...
title_en: "Lot and Serial Traceability"
title_ar: ""
domain: "ops"
...
domain_sources: ["modules/build09-trace-workspace.js", "platform/wms/index.mjs", "platform/wms/traceability-ops.mjs"]
tables_or_entities: ["wms_trace_profiles", "stock_lots", "stock_serials", "stock_move_lines"]
test_sources: ["tests/build-09/trace-workspaces-browser.test.mjs", "tests/build-09/traceability-recall-domain.test.mjs", "tests/build-09/operational-32-page-matrix-chromium.test.mjs"]
...
- Page ID: `lot_serial_traceability`; English: Lot and Serial Traceability; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('lot_serial_traceability')`; page-specific renderer is `modules/build09-trace-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Trace a lot or serial through receipts, internal moves, production, delivery, and current quality/expiry state.
...
- Renderer: `modules/build09-trace-workspace.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `trace (lot_id or serial_id required)` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: read/query only in this workspace; profile mutation is domain-supported by traceability-ops.
...
- Renderer/domain source: `modules/build09-trace-workspace.js` → `traceability.queryTrace/upsertTraceProfile`.
- Persisted entities/tables: wms_trace_profiles, stock_lots, stock_serials, stock_move_lines.
...
- `read/query only in this workspace; profile mutation is domain-supported by traceability-ops`
...
- The page renders scoped records from `trace (lot_id or serial_id required)`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_trace_profiles, stock_lots, stock_serials, stock_move_lines and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: receiving, production_receipt, canonical inventory.
- Downstream: recall_analysis, expiration_queue.
...
- English label: Lot and Serial Traceability; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-lot_serial_traceability-GAP-001** (P1, ERROR_STATE) — identity mismatch between lot, serial, and product must be surfaced as a recoverable validation state
- **PAGE-lot_serial_traceability-GAP-002** (P1, TEST) — prove trace results are company scoped and include no unrelated lot/serial movements
...
- Canonical home: `lot_serial_traceability` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-trace-workspace.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/trace-workspaces-browser.test.mjs`
- `tests/build-09/traceability-recall-domain.test.mjs`
...
- `traceability.queryTrace` → `upsertTraceProfile`
...
- Lot and Serial Traceability page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
