...
export function recordUsage(db, input, ctx) {
  ...
  if (['suspended', 'expired', 'cancelled', 'archived'].includes(sub.status)) throw new Build11Error(`Protected usage mutation is blocked while subscription is ${sub.status}.`, 'SUBSCRIPTION_STATE_BLOCKED', { status: sub.status }, 403);
...
function scopedOverviewMetric(db, table, column, value, tenant) {
  const tenantSql = tenant ? ' AND tenant_id=?' : '';
  const args = tenant ? [value, tenant] : [value];
  return Number(db.prepare(`SELECT COUNT(*) AS n FROM ${table} WHERE ${column}=?${tenantSql}`).get(...args)?.n || 0);
}

function buildOverview(db, tenant) {
  const scope = tenant ? 'WHERE tenant_id=?' : '';
  const args = tenant ? [tenant] : [];
  const lifecycle = db.prepare(`SELECT lifecycle_state AS state,COUNT(*) AS count FROM saas_tenant_profiles ${scope} GROUP BY lifecycle_state ORDER BY lifecycle_state`).all(...args);
  const card = (key, label, count, target, rows = []) => ({ key, label, count: Number(count || 0), target, rows: rows.slice(0, 5) });
  const trialRows = db.prepare(`SELECT s.tenant_id,t.name,s.trial_end_at FROM saas_subscriptions s JOIN platform_tenants t ON t.id=s.tenant_id WHERE s.status='trial' AND s.trial_end_at<=datetime('now','+14 day')${tenant ? ' AND s.tenant_id=?' : ''} ORDER BY s.trial_end_at LIMIT 5`).all(...args);
  const graceRows = db.prepare(`SELECT s.tenant_id,t.name,s.grace_end_at FROM saas_subscriptions s JOIN platform_tenants t ON t.id=s.tenant_id WHERE s.status='grace'${tenant ? ' AND s.tenant_id=?' : ''} ORDER BY s.grace_end_at LIMIT 5`).all(...args);
  const suspendedRows = db.prepare(`SELECT p.tenant_id,t.name,p.updated_at FROM saas_tenant_profiles p JOIN platform_tenants t ON t.id=p.tenant_id WHERE p.lifecycle_state='suspended'${tenant ? ' AND p.tenant_id=?' : ''} ORDER BY p.updated_at DESC LIMIT 5`).all(...args);
  const overageRows = db.prepare(`SELECT tenant_id,user_id,seat_type,status FROM saas_seat_assignments WHERE status='excess'${tenant ? ' AND tenant_id=?' : ''} ORDER BY assigned_at DESC LIMIT 5`).all(...args);
  const quotaRows = db.prepare(`SELECT tenant_id,metric,consumed,allowance,warning_threshold,policy FROM saas_usage_counters WHERE warning_threshold IS NOT NULL AND consumed>=warning_threshold${tenant ? ' AND tenant_id=?' : ''} ORDER BY updated_at DESC LIMIT 5`).all(...args);
  const failedProvisioningRows = db.prepare(`SELECT tenant_id,provisioning_error,updated_at FROM saas_tenant_profiles WHERE lifecycle_state='provisioning_failed'${tenant ? ' AND tenant_id=?' : ''} ORDER BY updated_at DESC LIMIT 5`).all(...args);
  const overdueRows = db.prepare(`SELECT tenant_id,id,total_amount,currency,status FROM saas_simulated_invoices WHERE status='overdue_simulated'${tenant ? ' AND tenant_id=?' : ''} ORDER BY updated_at DESC LIMIT 5`).all(...args);
  const validationRows = db.prepare(`SELECT package_id,name,version,validation_findings FROM saas_extension_packages WHERE publication_state='validation_failed' ORDER BY updated_at DESC LIMIT 5`).all();
  const reviewRows = db.prepare(`SELECT package_id,name,version,review_state,updated_at FROM saas_extension_packages WHERE review_state='pending' ORDER BY updated_at DESC LIMIT 5`).all();
  const countFor = (sql) => Number(db.prepare(sql).get(...args)?.n || 0);
  return {
    generated_at: new Date().toISOString(),
    permission_state: 'server_scoped',
    lifecycle: lifecycle.map((row) => ({ ...row, count: Number(row.count) })),
    metrics: [
      card('tenants_by_lifecycle', 'Tenants by lifecycle', lifecycle.reduce((sum, row) => sum + Number(row.count), 0), 'tenants', lifecycle),
      card('trials_ending_soon', 'Trials ending soon', trialRows.length ? countFor(`SELECT COUNT(*) AS n FROM saas_subscriptions WHERE status='trial' AND trial_end_at<=datetime('now','+14 day')${tenant ? ' AND tenant_id=?' : ''}`) : 0, 'subscriptions', trialRows),
      card('subscriptions_in_grace', 'Subscriptions in grace', countFor(`SELECT COUNT(*) AS n FROM saas_subscriptions WHERE status='grace'${tenant ? ' AND tenant_id=?' : ''}`), 'subscriptions', graceRows),
      card('suspended_tenants', 'Suspended tenants', countFor(`SELECT COUNT(*) AS n FROM saas_tenant_profiles WHERE lifecycle_state='suspended'${tenant ? ' AND tenant_id=?' : ''}`), 'tenants', suspendedRows),
      card('seat_overages', 'Seat overages', countFor(`SELECT COUNT(*) AS n FROM saas_seat_assignments WHERE status='excess'${tenant ? ' AND tenant_id=?' : ''}`), 'seats', overageRows),
      card('quotas_near_limit', 'Quotas near limit', quotaRows.length ? countFor(`SELECT COUNT(*) AS n FROM saas_usage_counters WHERE warning_threshold IS NOT NULL AND consumed>=warning_threshold${tenant ? ' AND tenant_id=?' : ''}`) : 0, 'usage-and-quotas', quotaRows),
      card('hard_quota_denials', 'Hard-quota denials', countFor(`SELECT COUNT(*) AS n FROM saas_quota_warnings WHERE warning_type='overage'${tenant ? ' AND tenant_id=?' : ''}`), 'usage-events'),
      card('failed_provisioning', 'Failed provisioning', countFor(`SELECT COUNT(*) AS n FROM saas_tenant_profiles WHERE lifecycle_state='provisioning_failed'${tenant ? ' AND tenant_id=?' : ''}`), 'provisioning', failedProvisioningRows),
      card('simulated_invoices_overdue', 'Simulated invoices overdue', countFor(`SELECT COUNT(*) AS n FROM saas_simulated_invoices WHERE status='overdue_simulated'${tenant ? ' AND tenant_id=?' : ''}`), 'billing-simulator', overdueRows),
      card('extension_validation_failures', 'Extension validation failures', Number(db.prepare("SELECT COUNT(*) AS n FROM saas_extension_packages WHERE publication_state='validation_failed'").get()?.n || 0), 'extension-marketplace', validationRows),
      card('pending_package_reviews', 'Pending package reviews', Number(db.prepare("SELECT COUNT(*) AS n FROM saas_extension_packages WHERE review_state='pending'").get()?.n || 0), 'extension-marketplace', reviewRows),
    ],
  };
}

export function listSaas(db, ctx, resource, recordId = null, query = {}, { crossTenant = false } = {}) {
  const tenant = crossTenant ? (query.tenant_id || query.tenantId || null) : ctx?.tenantId; const where = tenant ? 'WHERE tenant_id=?' : ''; const args = tenant ? [tenant] : [];
  if (resource === 'overview') return { data: [buildOverview(db, tenant)] };
  if (resource === 'tenants' || resource === 'tenant-directory') {
    const filters = []; const params = [];
    if (tenant) { filters.push('p.tenant_id=?'); params.push(tenant); }
    if (query.search || query.q) { filters.push('(LOWER(t.name) LIKE ? OR LOWER(p.tenant_id) LIKE ?)'); const search = `%${String(query.search || query.q).toLowerCase()}%`; params.push(search, search); }
    if (query.lifecycle_state) { filters.push('p.lifecycle_state=?'); params.push(query.lifecycle_state); }
    if (query.deployment_profile) { filters.push('p.deployment_profile=?'); params.push(query.deployment_profile); }
    if (query.plan_id) { filters.push('EXISTS (SELECT 1 FROM saas_subscriptions s JOIN saas_plan_versions pv ON pv.id=s.plan_version_id WHERE s.tenant_id=p.tenant_id AND pv.plan_id=?)'); params.push(query.plan_id); }
    return { data: db.prepare(`SELECT p.tenant_id AS id,t.name,p.deployment_profile,p.lifecycle_state,p.primary_company_id,p.support_status,p.provisioning_step,p.updated_at,(SELECT MAX(occurred_at) FROM saas_tenant_events e WHERE e.tenant_id=p.tenant_id) AS last_activity,(SELECT status FROM saas_subscriptions s WHERE s.tenant_id=p.tenant_id ORDER BY s.updated_at DESC LIMIT 1) AS subscription_state FROM saas_tenant_profiles p JOIN platform_tenants t ON t.id=p.tenant_id ${filters.length ? `WHERE ${filters.join(' AND ')}` : ''} ORDER BY t.name LIMIT 200`).all(...params) };
  }
  if (resource === 'tenant' && !recordId) return { data: crossTenant ? listSaas(db, ctx, 'tenants', null, query, { crossTenant: true }).data : profile(db, tenant) };
  ...
  if (resource === 'provisioning') return { data: db.prepare(`SELECT * FROM saas_tenant_provisioning ${where} ORDER BY started_at DESC LIMIT 200`).all(...args) };
  if (resource === 'tenant-events') return { data: db.prepare(`SELECT * FROM saas_tenant_events ${where} ORDER BY occurred_at DESC LIMIT 200`).all(...args) };
  ...
  if (resource === 'subscription-history') return { data: db.prepare(`SELECT h.*,s.tenant_id FROM saas_subscription_history h JOIN saas_subscriptions s ON s.id=h.subscription_id ${tenant ? 'WHERE s.tenant_id=?' : ''} ORDER BY h.occurred_at DESC LIMIT 200`).all(...args) };
  if (resource === 'entitlements') { const targetTenant = tenant || query.tenant_id; if (!targetTenant) return { data: [] }; const sub = planForSubscription(db, targetTenant); return { data: sub ? db.prepare('SELECT e.capability,\'plan\' AS source,? AS policy_state,? AS effective_from,? AS effective_until,\'ENTITLED\' AS reason_code,\'Capability is included by the active plan.\' AS explanation FROM saas_plan_entitlements e WHERE e.plan_version_id=? ORDER BY e.capability').all(sub.status, sub.current_period_start, sub.current_period_end, sub.plan_version_id) : [] }; }
  if (resource === 'entitlement-explain') { const targetTenant = tenant || query.tenant_id; const capability = safeString(query.capability, 'capability'); if (!targetTenant) return { error: 'tenant scope is required', status: 403 }; return { data: evaluateEntitlement(db, { ...ctx, tenantId: targetTenant }, capability, { mutation: query.mutation !== 'false' }) }; }
  ...
  if (resource === 'extension-history') return { data: db.prepare(`SELECT h.*,i.tenant_id FROM saas_extension_history h JOIN saas_extension_installations i ON i.id=h.installation_id ${tenant ? 'WHERE i.tenant_id=?' : ''} ORDER BY h.occurred_at DESC LIMIT 200`).all(...args) };
