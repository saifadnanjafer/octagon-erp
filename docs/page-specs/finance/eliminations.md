...
renderer_type: "view"
renderer_sources: ["views/eliminations.html"]
view_sources: ["views/eliminations.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-08/build08-page-contract.test.mjs","tests/build-08/consolidation-domain.test.mjs","tests/build-08/group-finance-browser-chromium.test.mjs"]
...
- Page ID: `eliminations`
- English: Eliminations
- Arabic: · قيود الاستبعاد
- Domain: `finance`; navigation group: `finance_consolidation`
- Route: switchPage('eliminations') / views/eliminations.html
- Page type: PAGE

# 2. Business Purpose

Consolidation eliminations — finance/consolidation (risk: critical, phase: build08)
...
User opens this page to work with the Eliminations workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/eliminations.html. Static source counts at baseline: 1 headings, 0 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/eliminations.html
...
- UI → renderer: views/eliminations.html
...
The intended output is the page’s named Eliminations record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: · قيود الاستبعاد
- English label: Eliminations
...
- Canonical candidate: `eliminations` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `eliminations` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Eliminations workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-08/build08-page-contract.test.mjs, tests/build-08/consolidation-domain.test.mjs, tests/build-08/group-finance-browser-chromium.test.mjs
...
- Eliminations workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
