...
tables_or_entities: ["wms_recall_cases", "wms_recall_impacts"]
test_sources: ["tests/build-09/traceability-recall-domain.test.mjs", "tests/build-09/trace-workspaces-browser.test.mjs", "tests/build-09/operational-32-page-matrix-chromium.test.mjs"]
...
- Page ID: `recall_analysis`; English: Recall Analysis; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('recall_analysis')`; page-specific renderer is `modules/build09-trace-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Identify a recall case from a lot/serial, calculate affected inventory/orders/production impacts, and propose holds.
...
- Data loading: `recall-cases` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:recall_identify, wms:recall_analyze, wms:recall_propose_holds.
...
- Renderer/domain source: `modules/build09-trace-workspace.js` → `traceability.identifyRecall/analyzeRecall/proposeRecallHolds/listRecallCases`.
- Persisted entities/tables: wms_recall_cases, wms_recall_impacts.
...
- The page renders scoped records from `recall-cases`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_recall_cases, wms_recall_impacts and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: lot_serial_traceability, expiration_queue.
- Downstream: quality_hold_queue and operational work items.
...
- English label: Recall Analysis; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-recall_analysis-GAP-001** (P1, WORKFLOW) — impact analysis and hold proposal must be separately evidenced
- **PAGE-recall_analysis-GAP-002** (P1, DATA) — duplicate recall reference/idempotency and closed-case behavior need direct tests
...
- Canonical home: `recall_analysis` unless a later owner decision explicitly changes the navigation contract.
...
- `traceability.identifyRecall` → `analyzeRecall` → `proposeRecallHolds` → `listRecallCases`
...
- Recall Analysis page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
