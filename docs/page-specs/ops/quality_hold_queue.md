---
page_id: "quality_hold_queue"
title_en: "Quality Hold Queue"
title_ar: "طابور تعليق الجودة"
domain: "ops"
navigation_group: "ops_production"
kind: QUEUE
canonical_status: PRIMARY
canonical_home: "quality_hold_queue"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
renderer_sources: ["modules/build09-quality-workspace.js"]
view_sources: []
api_sources: []
domain_sources: ["modules/build09-quality-workspace.js","services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-09/build09-workspaces-contract.test.mjs","tests/build-09/build09r2-bespoke-contract.test.mjs","tests/build-09/operational-32-page-matrix-chromium.test.mjs","tests/build-09/operational-production-quality-chromium.test.mjs","tests/build-09/quality-workspaces-browser.test.mjs"]
...
- Page ID: `quality_hold_queue`
- English: Quality Hold Queue
- Arabic: طابور تعليق الجودة
- Domain: `ops`; navigation group: `ops_production`
- Route: switchPage('quality_hold_queue') / views/quality_hold_queue.html
- Page type: QUEUE

# 2. Business Purpose

Quality hold queue — workshop/quality (risk: high, phase: build09)
...
User opens this page to work with the Quality Hold Queue workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/quality_hold_queue.html. Static source counts at baseline: 10 headings, 10 button tags, 0 input/select/textarea controls, 0 tables, 4 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: modules/build09-quality-workspace.js
...
- UI → renderer: modules/build09-quality-workspace.js
...
- UI label: ${esc(row.productId)} ${esc(row.checkpointType)} · ${esc(row.sourceType)}/${esc(row.sourceId)} · ${esc(when(row.createdAt))} ${esc(t('rejected', 'مرفوض'))} ${esc(num(row.rejectedQuantity))} ${row.ncrId ? badge('NCR', 'danger') : ''}${badge(row.status, CHECKPOINT_TONE[row.status] ?? '')}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ↻ ${esc(t('Sync with canonical inspection', 'مزامنة مع الفحص الرسمي'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Back to queue', 'العودة للطابور'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Request disposition', 'طلب القرار'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Conditionally accept', 'قبول مشروط'))} (${esc(t('max', 'حد أقصى'))} ${esc(num(current.sampleSize))}); handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
- UI label: ${esc(row.dispositionType)} ${esc(t('qty', 'كمية'))} ${esc(num(row.quantity))} · ${esc(row.reasonCode || '—')} · ${esc(t('by', 'بواسطة'))} ${esc(row.requestedBy || '—')} ${badge(row.status, DISPOSITION_TONE[row.status] ?? '')}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Approve disposition', 'اعتماد القرار'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Send canonical scrap request', 'إرسال طلب الإتلاف الرسمي'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Quality Hold Queue record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.

# 10. Workflow Position

- Upstream: NOT VERIFIED from explicit workflow metadata.
- Downstream: `rework_workspace`, `scrap_approval`
...
- Arabic label: طابور تعليق الجودة
- English label: Quality Hold Queue
...
- Canonical candidate: `quality_hold_queue` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `quality_hold_queue` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Quality Hold Queue workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-09/build09-workspaces-contract.test.mjs, tests/build-09/build09r2-bespoke-contract.test.mjs, tests/build-09/operational-32-page-matrix-chromium.test.mjs, tests/build-09/operational-production-quality-chromium.test.mjs, tests/build-09/quality-workspaces-browser.test.mjs
...
- Quality Hold Queue workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
