---
page_id: "configuration_profiles"
title_en: "Configuration Profiles"
title_ar: "ملفات الإعداد"
domain: "resources"
navigation_group: "resources_devices"
kind: SETUP
canonical_status: PRIMARY
canonical_home: "configuration_profiles"
parent_page: null
aliases: []
...
- Page ID: `configuration_profiles`
- English: Configuration Profiles
- Arabic: ملفات الإعداد
- Domain: `resources`; navigation group: `resources_devices`
- Route: switchPage('configuration_profiles') / views/configuration_profiles.html
- Page type: SETUP

# 2. Business Purpose

Configuration profiles — iot/config (risk: high, phase: build10)
...
User opens this page to work with the Configuration Profiles workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/configuration_profiles.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Configuration Profiles record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: ملفات الإعداد
- English label: Configuration Profiles
...
- Canonical candidate: `configuration_profiles` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `configuration_profiles` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Configuration Profiles workflow; the current 231/231 click audit is route activation evidence only.
...
- Configuration Profiles workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
