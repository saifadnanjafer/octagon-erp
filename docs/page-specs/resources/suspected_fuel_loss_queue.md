---
page_id: "suspected_fuel_loss_queue"
title_en: "Suspected Fuel Loss Queue"
title_ar: "طابور فاقد الوقود المشتبه"
domain: "resources"
navigation_group: "resources_fleet"
kind: QUEUE
canonical_status: PRIMARY
canonical_home: "suspected_fuel_loss_queue"
parent_page: null
aliases: []
...
- Page ID: `suspected_fuel_loss_queue`
- English: Suspected Fuel Loss Queue
- Arabic: طابور فاقد الوقود المشتبه
- Domain: `resources`; navigation group: `resources_fleet`
- Route: switchPage('suspected_fuel_loss_queue') / views/suspected_fuel_loss_queue.html
- Page type: QUEUE

# 2. Business Purpose

Suspected fuel loss queue — fleet/fuel (risk: high, phase: build10)
...
User opens this page to work with the Suspected Fuel Loss Queue workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/suspected_fuel_loss_queue.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Suspected Fuel Loss Queue record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: طابور فاقد الوقود المشتبه
- English label: Suspected Fuel Loss Queue
...
- Canonical candidate: `suspected_fuel_loss_queue` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `suspected_fuel_loss_queue` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Suspected Fuel Loss Queue workflow; the current 231/231 click audit is route activation evidence only.
...
- Suspected Fuel Loss Queue workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
