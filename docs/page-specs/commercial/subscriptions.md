...
canonical_home: "subscriptions"
parent_page: null
aliases: []
roles: ["system.admin","finance.manager"]
permission: "system.admin, finance.manager"
entitlement: "UNMAPPED — commercial/SaaS page, no literal saas_plan_entitlements \"page:*\" row found"
renderer_type: "javascript"
renderer_sources: ["views/subscriptions.html","modules/build11-workspaces.js"]
view_sources: ["views/subscriptions.html"]
...
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-11/admin-workspaces-contract.test.mjs","tests/build-11/build11r-acceptance-contract.test.mjs","tests/build-11/commercial-platform-browser.test.mjs","tests/build-11/commercial-platform.test.mjs"]
...
- Route: switchPage('subscriptions') / views/subscriptions.html
- Page type: PAGE

# 2. Business Purpose

Subscriptions — commercial/subscriptions (risk: high, phase: build11)

# 3. Primary Users

- Declared roles: system.admin, finance.manager
- Viewer/operator/reviewer/manager/administrator split: NOT VERIFIED beyond the declared permission gate.

# 4. Primary User Goal

User opens this page to work with the Subscriptions workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/subscriptions.html. Static source counts at baseline: 6 headings, 6 button tags, 7 input/select/textarea controls, 1 tables, 1 forms, and 2 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/subscriptions.html
...
- UI → renderer: views/subscriptions.html
...
The intended output is the page’s named Subscriptions record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.

# 10. Workflow Position

- Upstream: NOT VERIFIED from explicit workflow metadata.
- Downstream: `entitlements`, `events`
...
- Client page gate: system.admin, finance.manager
- Entitlement: UNMAPPED — commercial/SaaS page, no literal saas_plan_entitlements "page:*" row found
...
- Arabic label: الاشتراكات
- English label: Subscriptions
...
- Canonical candidate: `subscriptions` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `subscriptions` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Subscriptions workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-11/admin-workspaces-contract.test.mjs, tests/build-11/build11r-acceptance-contract.test.mjs, tests/build-11/commercial-platform-browser.test.mjs, tests/build-11/commercial-platform.test.mjs
...
- Subscriptions workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
