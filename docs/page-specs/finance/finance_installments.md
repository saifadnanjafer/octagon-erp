...
renderer_type: "view"
renderer_sources: ["views/finance_installments.html"]
...
- Page ID: `finance_installments`
- English: Finance Installments
- Arabic: خطط التقسيط
- Domain: `finance`; navigation group: `finance_accounts`
- Route: switchPage('finance_installments') / views/finance_installments.html
- Page type: PAGE

# 2. Business Purpose

Finance installments — finance/installments (risk: high, phase: phase7j)

# 3. Primary Users

- Declared roles: finance.manager, finance.user
- Viewer/operator/reviewer/manager/administrator split: NOT VERIFIED beyond the declared permission gate.

# 4. Primary User Goal

User opens this page to work with the Finance Installments workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/finance_installments.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/finance_installments.html
...
- UI → renderer: views/finance_installments.html
...
The intended output is the page’s named Finance Installments record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Client page gate: finance.manager, finance.user
- Entitlement: none/global (not a commercial/SaaS-gated page)
...
- Arabic label: خطط التقسيط
- English label: Finance Installments
...
- Canonical candidate: `finance_installments` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `finance_installments` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Finance Installments workflow; the current 231/231 click audit is route activation evidence only.
...
- Finance Installments workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
