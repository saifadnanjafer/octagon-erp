...
domain_sources: ["modules/build09-production-material-workspaces.js", "platform/wms/index.mjs", "platform/manufacturing/material-flow.mjs"]
tables_or_entities: ["shopfloor_material_requests", "shopfloor_material_request_lines"]
test_sources: ["tests/build-09/production-material-workspaces-browser.test.mjs", "tests/build-09/production-material-flow-domain.test.mjs", "tests/build-09/production-material-workspaces-contract.test.mjs"]
...
- Page ID: `production_material_requests`; English: Production Material Requests; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('production_material_requests')`; page-specific renderer is `modules/build09-production-material-workspaces.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Request components for a production operation, check availability, and approve the material release.
...
- Renderer: `modules/build09-production-material-workspaces.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `material-flow` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: shopfloor:material_request, shopfloor:material_availability, shopfloor:material_approve.
...
- Renderer/domain source: `modules/build09-production-material-workspaces.js` → `materialFlow.listMaterialFlowRequests/materialShortageBoard and request/availability/approval handlers`.
- Persisted entities/tables: shopfloor_material_requests, shopfloor_material_request_lines.
...
- `shopfloor:material_request`
- `shopfloor:material_availability`
- `shopfloor:material_approve`
...
- The page renders scoped records from `material-flow`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into shopfloor_material_requests, shopfloor_material_request_lines and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: shopfloor_terminal, inventory availability.
- Downstream: production_issue_return, canonical material move.
...
- English label: Production Material Requests; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-production_material_requests-GAP-001** (P1, WORKFLOW) — approval is distinct from canonical issue and must remain so in the spec
- **PAGE-production_material_requests-GAP-002** (P1, DATA) — shortage and partial-availability fixture coverage must be explicit
...
- Canonical home: `production_material_requests` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-production-material-workspaces.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/production-material-workspaces-browser.test.mjs`
- `tests/build-09/production-material-flow-domain.test.mjs`
- `tests/build-09/production-material-workspaces-contract.test.mjs`
...
- `materialFlow.listMaterialFlowRequests` → `materialShortageBoard and request` → `availability` → `approval handlers`
...
- Production Material Requests page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
