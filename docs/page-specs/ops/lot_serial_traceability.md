---
page_id: "lot_serial_traceability"
title_en: "Lot / Serial Traceability"
title_ar: "تتبع الدفعات والأرقام التسلسلية"
domain: "ops"
navigation_group: "ops_warehouse_traceability"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "lot_serial_traceability"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
renderer_sources: ["modules/build09-expiration-workspace.js"]
view_sources: []
api_sources: []
domain_sources: ["modules/build09-expiration-workspace.js","services/permissionService.js","modules/build09-production-material-workspaces.js","modules/build09-trace-workspace.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-09/build09-workspaces-contract.test.mjs","tests/build-09/build09r2-bespoke-contract.test.mjs","tests/build-09/expiration-queue-browser.test.mjs","tests/build-09/operational-32-page-matrix-chromium.test.mjs","tests/build-09/production-material-workspaces-browser.test.mjs","tests/build-09/trace-workspaces-browser.test.mjs"]
...
- Page ID: `lot_serial_traceability`
- English: Lot / Serial Traceability
- Arabic: تتبع الدفعات والأرقام التسلسلية
- Domain: `ops`; navigation group: `ops_warehouse_traceability`
- Route: switchPage('lot_serial_traceability') / views/lot_serial_traceability.html
- Page type: PAGE

# 2. Business Purpose

Lot and serial traceability — workshop/inventory (risk: high, phase: build09)
...
User opens this page to work with the Lot / Serial Traceability workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/lot_serial_traceability.html. Static source counts at baseline: 13 headings, 14 button tags, 6 input/select/textarea controls, 0 tables, 4 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: modules/build09-expiration-workspace.js
...
- UI → renderer: modules/build09-expiration-workspace.js
...
- UI label: ${esc(t('Trace', 'تتبع'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Propose Quality disposition', 'اقتراح تصرف الجودة'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(label(row.product, row.productId))} ${esc(label(row.productionOrder, row.productionOrderId))} · ${esc(label(row.workOrder, row.workOrderId))} ${statusBadge(row.status)}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Create governed request', 'إنشاء طلب محكوم'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Check availability','فحص التوفر'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Approve proposal','اعتماد المقترح'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Request canonical movement','طلب الحركة الرسمية'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
- UI label: ${esc(t('Open traceability','فتح التتبع'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(row.reference)} ${esc(row.reason)} · ${esc(when(row.identifiedAt))} ${statusBadge(row.status)}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Identify recall case', 'تحديد حالة السحب'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Analyze impact', 'تحليل التأثير'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Lot / Serial Traceability record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.

# 10. Workflow Position

- Upstream: NOT VERIFIED from explicit workflow metadata.
- Downstream: `expiration_queue`, `production_issue_return`, `production_material_requests`, `production_receipt`, `recall_analysis`
...
- Arabic label: تتبع الدفعات والأرقام التسلسلية
- English label: Lot / Serial Traceability
...
- Canonical candidate: `lot_serial_traceability` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `lot_serial_traceability` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Lot / Serial Traceability workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-09/build09-workspaces-contract.test.mjs, tests/build-09/build09r2-bespoke-contract.test.mjs, tests/build-09/expiration-queue-browser.test.mjs, tests/build-09/operational-32-page-matrix-chromium.test.mjs, tests/build-09/production-material-workspaces-browser.test.mjs, tests/build-09/trace-workspaces-browser.test.mjs
...
- Lot / Serial Traceability workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
