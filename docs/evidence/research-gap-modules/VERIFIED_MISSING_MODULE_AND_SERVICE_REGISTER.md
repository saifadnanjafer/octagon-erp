...
| 10 | Returns/RMA/repair (§7.11) | PAGE ONLY | `platform/inventory`, `platform/quality` (existing, unwired) | **Yes — `modules/warranty-rma.js` mock vs real inventory-return command vs real NCR/CAPA are three disconnected "truths" for the same business event** | No — P1, but flagged HIGH priority for next wave given the 3-way duplication risk |
...
| 18 | Collaboration/Chatter wiring (§7.1) | REGISTERED BUT UNREACHABLE | `platform/collaboration` (existing) | **Yes — same defect class as jobs (#1).** Not built this wave to keep the P0 slice coherent and reviewable on its own. | No — **explicit P0-tier candidate for the very next slice**, since it is now the single remaining "fully built, tested, zero-risk, pure-wiring" item of its kind |

## Build queue for continuation (dependency order, P0 first)

1. ~~Scheduler/Jobs (#1)~~ — **done this wave.**
2. Collaboration/Chatter wiring (#18) — same fix pattern, same near-zero risk, unlocks entity-timeline UI for every other module.
3. Notifications provider completion (#3) — now that jobs exist, retry/dead-letter delivery can be driven by the job queue instead of ad hoc.
4. Report dispatch (#2) — depends on #1 (needs the job queue, now available) and a new rendering/export capability.
5. Returns/RMA consolidation (#10) — highest business risk of silent data disagreement (3 competing "truths"); needs design work, not just wiring.
6. Credit/collections UI (#8), Print/template consolidation (#4), Sales commission rewire (#7) — can proceed in parallel, independent of each other.
7. Master data governance (#6), Service entitlements (#11) — genuinely new modules; scope/design needed before building.
8. IoT (#13), Kiosk (#14), Marketplace (#15), Workshop pack (#17), Tenant provisioning (#16) — lower priority per the source matrix's own phase assignments (06/08/post-GA).
