...
renderer_type: "view"
renderer_sources: ["views/mps.html"]
view_sources: ["views/mps.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-08/build08-page-contract.test.mjs","tests/build-08/mps-sop-domain.test.mjs","tests/build-08/planning-browser-chromium.test.mjs","tests/build-08/planning-query-action-contract.test.mjs","tests/build-08/platform-governance-contract.test.mjs","tests/cutover/_staged-clone.mjs","tests/phase03/finance-browser-evidence.test.mjs"]
...
- Page ID: `mps`
- English: Mps
- Arabic: · جدول الإنتاج الرئيسي
- Domain: `ops`; navigation group: `ops_planning`
- Route: switchPage('mps') / views/mps.html
- Page type: PAGE

# 2. Business Purpose

Master production schedule — planning/production (risk: high, phase: build08)
...
User opens this page to work with the Mps workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/mps.html. Static source counts at baseline: 1 headings, 0 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/mps.html
...
The intended output is the page’s named Mps record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: · جدول الإنتاج الرئيسي
- English label: Mps
...
- Canonical candidate: `mps` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `mps` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Mps workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-08/build08-page-contract.test.mjs, tests/build-08/mps-sop-domain.test.mjs, tests/build-08/planning-browser-chromium.test.mjs, tests/build-08/planning-query-action-contract.test.mjs, tests/build-08/platform-governance-contract.test.mjs, tests/cutover/_staged-clone.mjs, tests/phase03/finance-browser-evidence.test.mjs
...
- Mps workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
