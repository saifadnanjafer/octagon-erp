---
page_id: "operational_performance"
title_en: "Operational Performance"
title_ar: ""
domain: "ops"
navigation_group: "ops_inventory"
kind: ANALYTICS
canonical_status: "PRIMARY"
canonical_home: "operational_performance"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
renderer_sources: ["modules/build09-downtime-workspace.js"]
view_sources: []
api_sources: ["platform/api/build09.mjs"]
domain_sources: ["modules/build09-downtime-workspace.js", "platform/wms/index.mjs", "platform/manufacturing/downtime-performance.mjs"]
tables_or_entities: ["shopfloor_sessions", "shopfloor_outputs", "shopfloor_downtime_events"]
test_sources: ["tests/build-09/downtime-workspaces-browser.test.mjs", "tests/build-09/shopfloor-downtime-domain.test.mjs", "tests/build-09/operational-production-quality-chromium.test.mjs"]
...
- Page ID: `operational_performance`; English: Operational Performance; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('operational_performance')`; page-specific renderer is `modules/build09-downtime-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Show throughput, rejects, queue, downtime, and only those rate metrics for which the server has reliable evidence.
...
- Renderer: `modules/build09-downtime-workspace.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `work-center-performance, shopfloor-sessions` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: read-only analytics; no mutation action exposed.
...
- Renderer/domain source: `modules/build09-downtime-workspace.js` → `performance.workCenterPerformance and evidence-based metric calculation`.
- Persisted entities/tables: shopfloor_sessions, shopfloor_outputs, shopfloor_downtime_events.
...
- `read-only analytics; no mutation action exposed`
...
- The page renders scoped records from `work-center-performance, shopfloor-sessions`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into shopfloor_sessions, shopfloor_outputs, shopfloor_downtime_events and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: shopfloor_terminal, downtime_board.
- Downstream: operational review and planning.
...
- English label: Operational Performance; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-operational_performance-GAP-001** (P1, DATA) — null/unmeasured rates must remain “not available”, never zero or 100%
- **PAGE-operational_performance-GAP-002** (P1, FIXTURE) — performance evidence needs completed-session fixtures with planned windows, timing, and output
...
- Canonical home: `operational_performance` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-downtime-workspace.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/downtime-workspaces-browser.test.mjs`
- `tests/build-09/shopfloor-downtime-domain.test.mjs`
...
- Operational Performance page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
