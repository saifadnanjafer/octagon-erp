...
renderer_type: "view"
renderer_sources: ["views/procurement.html"]
view_sources: ["views/procurement.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-08/mps-sop-domain.test.mjs","tests/checkpoint-c/canonical_procurement_ui.test.mjs","tests/checkpoint-c/canonical_work_management_ui.test.mjs","tests/checkpoint-c/migration_047.test.mjs","tests/checkpoint-c/migration_049.test.mjs","tests/checkpoint-c/procurement_lifecycle.test.mjs","tests/checkpoint-c/work_item_lifecycle.test.mjs","tests/checkpoint-d-e/engineering_bom_routing_mrp.test.mjs"]
...
- Route: switchPage('procurement') / views/procurement.html
- Page type: PAGE

# 2. Business Purpose

Procurement — procurement/finance (risk: high, phase: phase6e)

# 3. Primary Users

- Declared roles: workshop.manager, finance.user
- Viewer/operator/reviewer/manager/administrator split: NOT VERIFIED beyond the declared permission gate.

# 4. Primary User Goal

User opens this page to work with the Procurement workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/procurement.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/procurement.html
...
- UI → renderer: views/procurement.html
...
The intended output is the page’s named Procurement record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Client page gate: workshop.manager, finance.user
- Entitlement: none/global (not a commercial/SaaS-gated page)
...
- Arabic label: المشتريات
- English label: Procurement
...
- Canonical candidate: `procurement` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `procurement` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Procurement workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-08/mps-sop-domain.test.mjs, tests/checkpoint-c/canonical_procurement_ui.test.mjs, tests/checkpoint-c/canonical_work_management_ui.test.mjs, tests/checkpoint-c/migration_047.test.mjs, tests/checkpoint-c/migration_049.test.mjs, tests/checkpoint-c/procurement_lifecycle.test.mjs, tests/checkpoint-c/work_item_lifecycle.test.mjs, tests/checkpoint-d-e/engineering_bom_routing_mrp.test.mjs
...
- Procurement workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
