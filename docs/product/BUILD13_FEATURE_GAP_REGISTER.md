...
### GAP-006 — `my_work` 404s on saved-views call (P1, open)
`platform/api/index.mjs:159` has a matching route, gated on a `configuration` service that IS wired in per `platform-runtime-bridge.mjs`. Root cause not isolated this pass — needs a live request trace, not yet attempted. `my_work` otherwise functions; this is one secondary feature within it.
