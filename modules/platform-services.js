...
(function () {
  ...
  async function render() {
    ...
    let inbox = [], activities = [], views = [], health = { providers: [], deadLetters: [] }, error = null;
    try { [inbox, activities, health, views] = await Promise.all([api('notifications'), api('activities'), api('notification-health'), api('saved-views?entity=sale_contract')]); } catch (e) { error = e.message; }
    ...
    section.innerHTML = '<div class="pmk-hero"><div><div class="pmk-kicker">Platform Services</div><h3>التعاون والإشعارات والأنشطة</h3><p>كل البيانات من API المحكوم؛ لا توجد كتابة مباشرة أو مزود خارجي مفعّل.</p></div><button class="btn-secondary" onclick="PlatformServices.refresh()">تحديث</button></div>'
      ...
      + '<div class="pmk-panel"><h3>Saved Views (' + views.length + ')</h3>' + (views.slice(0,6).map(v => '<p><strong>' + esc(v.name) + '</strong><br>' + esc(v.entity) + '</p>').join('') || '<p>لا توجد views محفوظة.</p>') + '</div>'
