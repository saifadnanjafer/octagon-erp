...
renderer_type: "view"
renderer_sources: ["views/account_mapping.html"]
view_sources: ["views/account_mapping.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-08/build08-page-contract.test.mjs","tests/phase04-finalization/canonical_client.test.mjs"]
...
- Page ID: `account_mapping`
- English: Account Mapping
- Arabic: · ربط الحسابات
- Domain: `finance`; navigation group: `finance_consolidation`
- Route: switchPage('account_mapping') / views/account_mapping.html
- Page type: PAGE

# 2. Business Purpose

Consolidation account mapping — finance/consolidation (risk: critical, phase: build08)
...
User opens this page to work with the Account Mapping workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/account_mapping.html. Static source counts at baseline: 1 headings, 0 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/account_mapping.html
...
- UI → renderer: views/account_mapping.html
...
The intended output is the page’s named Account Mapping record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: · ربط الحسابات
- English label: Account Mapping
...
- Canonical candidate: `account_mapping` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `account_mapping` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Account Mapping workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-08/build08-page-contract.test.mjs, tests/phase04-finalization/canonical_client.test.mjs
...
- Account Mapping workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
