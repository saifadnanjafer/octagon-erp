...
renderer_type: "view"
renderer_sources: ["views/sop.html"]
view_sources: ["views/sop.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-08/build08-page-contract.test.mjs","tests/build-08/planning-query-action-contract.test.mjs","tests/build-08/platform-governance-contract.test.mjs"]
...
- Route: switchPage('sop') / views/sop.html
- Page type: PAGE

# 2. Business Purpose

SOP library — workshop/quality (risk: low, phase: core)
...
User opens this page to work with the Sop workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/sop.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/sop.html
...
The intended output is the page’s named Sop record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Canonical candidate: `sop` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `sop` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Sop workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-08/build08-page-contract.test.mjs, tests/build-08/planning-query-action-contract.test.mjs, tests/build-08/platform-governance-contract.test.mjs
...
- Sop workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
