...
title_ar: "إدارة المناطق والخانات"
domain: "ops"
...
view_sources: []
api_sources: []
domain_sources: ["modules/build09-topology-workspace.js","services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-09/build09-workspaces-contract.test.mjs","tests/build-09/build09r2-bespoke-contract.test.mjs","tests/build-09/operational-32-page-matrix-chromium.test.mjs","tests/build-09/operational-topology-chromium.test.mjs","tests/build-09/topology-workspaces-browser.test.mjs","tests/workshop/readiness-guidance.test.mjs"]
...
- Page ID: `zone_bin_management`
- English: Zone and Bin Management
- Arabic: إدارة المناطق والخانات
- Domain: `ops`; navigation group: `ops_inventory`
- Route: switchPage('zone_bin_management') / views/zone_bin_management.html
- Page type: PAGE

# 2. Business Purpose

Zone and bin management — workshop/inventory (risk: high, phase: build09)
...
User opens this page to work with the Zone and Bin Management workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/zone_bin_management.html. Static source counts at baseline: 5 headings, 2 button tags, 0 input/select/textarea controls, 0 tables, 2 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: modules/build09-topology-workspace.js
...
- UI → renderer: modules/build09-topology-workspace.js
...
- UI label: ${esc(t('Create scoped zone', 'إنشاء منطقة ضمن النطاق'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: ${esc(t('Create bin profile', 'إنشاء ملف الخانة'))}; handler/action ID: NOT VERIFIED; permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Zone and Bin Management record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.

# 10. Workflow Position

- Upstream: NOT VERIFIED from explicit workflow metadata.
- Downstream: `warehouse_topology`
...
- Arabic label: إدارة المناطق والخانات
- English label: Zone and Bin Management
...
- Canonical candidate: `zone_bin_management` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `zone_bin_management` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Zone and Bin Management workflow; the current 231/231 click audit is route activation evidence only.
...
- Zone and Bin Management workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED.
