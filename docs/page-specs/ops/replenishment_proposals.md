...
domain_sources: ["modules/build09-putaway-workspace.js", "platform/wms/index.mjs", "platform/wms/putaway.mjs"]
tables_or_entities: ["wms_replenishment_proposals_v2", "wms_warehouse_tasks"]
test_sources: ["tests/build-09/putaway-workspaces-browser.test.mjs", "tests/build-09/wms-foundation-domain.test.mjs", "tests/build-09/cross-domain-lifecycles.test.mjs"]
...
implementation_status: "PARTIAL_AT_ENGINEERING_REFERENCE"
...
- Page ID: `replenishment_proposals`; English: Replenishment Proposals; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('replenishment_proposals')`; page-specific renderer is `modules/build09-putaway-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Review shortages and proposed moves, then approve/cancel/retry the proposal before canonical execution.
...
- Renderer: `modules/build09-putaway-workspace.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `replenishment-proposals` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:replenishment_calculate, wms:replenishment_cancel.
...
- Renderer/domain source: `modules/build09-putaway-workspace.js` → `replenishment.calculateReplenishment/approveReplenishment/cancelReplenishment/retryReplenishment/listReplenishmentProposals`.
- Persisted entities/tables: wms_replenishment_proposals_v2, wms_warehouse_tasks.
...
- `wms:replenishment_calculate`
- `wms:replenishment_cancel`
...
- The page renders scoped records from `replenishment-proposals`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_replenishment_proposals_v2, wms_warehouse_tasks and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: replenishment_rules, location stock.
- Downstream: putaway_task_queue, canonical inventory stock move.
...
- English label: Replenishment Proposals; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-replenishment_proposals-GAP-001** (P1, ACTION) — domain approval/retry paths must be reconciled with the visible proposal controls
- **PAGE-replenishment_proposals-GAP-002** (P1, WORKFLOW) — proposal-to-task-to-canonical completion needs a direct end-to-end proof
...
- Canonical home: `replenishment_proposals` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-putaway-workspace.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/putaway-workspaces-browser.test.mjs`
- `tests/build-09/wms-foundation-domain.test.mjs`
...
- `replenishment.calculateReplenishment` → `approveReplenishment` → `cancelReplenishment` → `retryReplenishment` → `listReplenishmentProposals`
...
- Replenishment Proposals page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
