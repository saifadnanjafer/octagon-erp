---
page_id: "intercompany_reconciliation"
title_en: "Intercompany Reconciliation"
title_ar: "· التسوية والمطابقة"
domain: "finance"
navigation_group: "finance_intercompany"
kind: TRANSACTION
canonical_status: PRIMARY
canonical_home: "intercompany_reconciliation"
parent_page: null
aliases: []
...
renderer_type: "view"
renderer_sources: ["views/intercompany_reconciliation.html"]
view_sources: ["views/intercompany_reconciliation.html"]
api_sources: []
...
- Page ID: `intercompany_reconciliation`
- English: Intercompany Reconciliation
- Arabic: · التسوية والمطابقة
- Domain: `finance`; navigation group: `finance_intercompany`
- Route: switchPage('intercompany_reconciliation') / views/intercompany_reconciliation.html
- Page type: TRANSACTION

# 2. Business Purpose

Intercompany reconciliation — finance/intercompany (risk: critical, phase: build08)
...
User opens this page to work with the Intercompany Reconciliation workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/intercompany_reconciliation.html. Static source counts at baseline: 1 headings, 0 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/intercompany_reconciliation.html
...
- UI → renderer: views/intercompany_reconciliation.html
...
The intended output is the page’s named Intercompany Reconciliation record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: · التسوية والمطابقة
- English label: Intercompany Reconciliation
...
- Canonical candidate: `intercompany_reconciliation` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `intercompany_reconciliation` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Intercompany Reconciliation workflow; the current 231/231 click audit is route activation evidence only.
...
- `views/intercompany_reconciliation.html`
- `index.html`
...
- Intercompany Reconciliation workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
