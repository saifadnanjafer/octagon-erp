...
title_en: "Production Issue and Return"
title_ar: ""
domain: "ops"
...
tables_or_entities: ["shopfloor_material_requests", "shopfloor_material_request_lines", "shopfloor_material_moves"]
test_sources: ["tests/build-09/production-material-workspaces-browser.test.mjs", "tests/build-09/production-material-flow-domain.test.mjs", "tests/build-09/cross-domain-lifecycles.test.mjs"]
...
- Page ID: `production_issue_return`; English: Production Issue and Return; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('production_issue_return')`; page-specific renderer is `modules/build09-production-material-workspaces.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Request canonical component issue or return, then acknowledge the resulting Inventory move.
...
- Primary actions exposed by this page: shopfloor:material_request_canonical, shopfloor:material_acknowledge.
...
- Renderer/domain source: `modules/build09-production-material-workspaces.js` → `materialFlow request/acknowledgement handlers with canonical stock authority`.
- Persisted entities/tables: shopfloor_material_requests, shopfloor_material_request_lines, shopfloor_material_moves.
...
- `shopfloor:material_request_canonical`
- `shopfloor:material_acknowledge`
...
- Mutations persist into shopfloor_material_requests, shopfloor_material_request_lines, shopfloor_material_moves and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: production_material_requests, shopfloor_terminal.
...
- English label: Production Issue and Return; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-production_issue_return-GAP-001** (P1, WORKFLOW) — issue and return are separate request types and need separate lifecycle evidence
- **PAGE-production_issue_return-GAP-002** (P1, PERMISSION) — acknowledgement must re-verify the canonical result belongs to the request and scope
...
- Canonical home: `production_issue_return` unless a later owner decision explicitly changes the navigation contract.
...
- `tests/build-09/cross-domain-lifecycles.test.mjs`
...
- `materialFlow request` → `acknowledgement handlers with canonical stock authority`
...
- Production Issue and Return page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
