...
title_en: "Offline Queue"
title_ar: "طابور العمل دون اتصال"
domain: "resources"
navigation_group: "resources_offline"
kind: QUEUE
canonical_status: PRIMARY
canonical_home: "offline_queue"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
- Page ID: `offline_queue`
- English: Offline Queue
- Arabic: طابور العمل دون اتصال
- Domain: `resources`; navigation group: `resources_offline`
- Route: switchPage('offline_queue') / views/offline_queue.html
- Page type: QUEUE

# 2. Business Purpose

Offline queue — offline/queue (risk: medium, phase: build10)
...
User opens this page to work with the Offline Queue workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/offline_queue.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Offline Queue record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: طابور العمل دون اتصال
- English label: Offline Queue
...
- Canonical candidate: `offline_queue` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `offline_queue` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Offline Queue workflow; the current 231/231 click audit is route activation evidence only.
...
- Offline Queue workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
