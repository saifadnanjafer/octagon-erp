...
renderer_type: "view"
renderer_sources: ["views/mrp.html"]
view_sources: ["views/mrp.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/checkpoint-d-e/engineering_bom_routing_mrp.test.mjs","tests/checkpoint-d-e/shell_dispatcher.test.mjs","tests/checkpoint-f/canonical_authority_coverage.test.mjs"]
...
- Page ID: `mrp`
- English: Mrp
- Arabic: تخطيط موارد التصنيع
- Domain: `ops`; navigation group: `ops_production`
- Route: switchPage('mrp') / views/mrp.html
- Page type: PAGE

# 2. Business Purpose

Manufacturing MRP II — workshop/production (risk: medium, phase: phase6h)
...
User opens this page to work with the Mrp workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/mrp.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/mrp.html
...
The intended output is the page’s named Mrp record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: تخطيط موارد التصنيع
- English label: Mrp
...
- Canonical candidate: `mrp` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `mrp` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Mrp workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/checkpoint-d-e/engineering_bom_routing_mrp.test.mjs, tests/checkpoint-d-e/shell_dispatcher.test.mjs, tests/checkpoint-f/canonical_authority_coverage.test.mjs
...
- Mrp workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
