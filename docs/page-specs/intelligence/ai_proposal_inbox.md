---
page_id: "ai_proposal_inbox"
title_en: "AI Proposal Inbox"
title_ar: "صندوق مقترحات الذكاء الاصطناعي"
domain: "intelligence"
navigation_group: "intelligence_governance"
kind: QUEUE
canonical_status: PRIMARY
canonical_home: "ai_proposal_inbox"
parent_page: null
aliases: []
...
- Page ID: `ai_proposal_inbox`
- English: AI Proposal Inbox
- Arabic: صندوق مقترحات الذكاء الاصطناعي
- Domain: `intelligence`; navigation group: `intelligence_governance`
- Route: switchPage('ai_proposal_inbox') / views/ai_proposal_inbox.html
- Page type: QUEUE

# 2. Business Purpose

AI proposal inbox — build12/ai-proposals (risk: high, phase: build12)
...
User opens this page to work with the AI Proposal Inbox workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/ai_proposal_inbox.html. Static source counts at baseline: 2 headings, 3 button tags, 2 input/select/textarea controls, 1 tables, 1 forms, and 2 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named AI Proposal Inbox record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: صندوق مقترحات الذكاء الاصطناعي
- English label: AI Proposal Inbox
...
The AI Proposal Inbox workspace should give its governed users a page-specific way to complete the named business task: load scoped records, accept governed inputs where needed, execute authorized actions, persist the resulting state through the domain authority, and expose explicit loading/empty/denied/validation/server-error/success states. Exact fields and lifecycle transitions remain **NOT VERIFIED** where the baseline source does not define them.

# 17. Identified Gaps

- **PAGE-ai_proposal_inbox-GAP-004** (P1, PERMISSION) — The review inventory flags this commercial/SaaS page as lacking a matching entitlement mapping in the disposable review database.
- **PAGE-ai_proposal_inbox-GAP-005** (P2, TEST) — Documentation contradiction: NAVIGATION_FORENSIC_REPORT.json records app.js switchPage, while the module registry also contains this page. Reconcile renderer ownership after Product Recovery.
...
- Canonical candidate: `ai_proposal_inbox` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `ai_proposal_inbox` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named AI Proposal Inbox workflow; the current 231/231 click audit is route activation evidence only.
...
- AI Proposal Inbox workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
