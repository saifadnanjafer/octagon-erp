...
tables_or_entities: ["wms_pick_waves", "wms_pick_wave_tasks"]
test_sources: ["tests/build-09/wave-workspaces-browser.test.mjs", "tests/build-09/mobile-picking-wave-domain.test.mjs", "tests/build-09/cross-domain-lifecycles.test.mjs"]
...
- Page ID: `wave_planning`; English: Wave Planning; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('wave_planning')`; page-specific renderer is `modules/build09-wave-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Select eligible ready/assigned/pending pick tasks, group them by strategy, calculate the wave, and submit it for independent review.
...
- Primary actions exposed by this page: wms:wave_create, wms:wave_calculate, wms:wave_review.
...
- Renderer/domain source: `modules/build09-wave-workspace.js` → `waves.createWave/calculateWave/reviewWave`.
- Persisted entities/tables: wms_pick_waves, wms_pick_wave_tasks.
...
- Mutations persist into wms_pick_waves, wms_pick_wave_tasks and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: pick_task_queue.
- Downstream: wave_execution, mobile_picking.
...
- English label: Wave Planning; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-wave_planning-GAP-001** (P1, PERMISSION) — creator/reviewer separation is enforced by domain logic and needs a browser evidence row
- **PAGE-wave_planning-GAP-002** (P1, WORKFLOW) — cutoff, staging, and recalculation behavior should be demonstrated against real task fixtures
...
- Canonical home: `wave_planning` unless a later owner decision explicitly changes the navigation contract.
...
- `waves.createWave` → `calculateWave` → `reviewWave`
...
- Wave Planning page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
