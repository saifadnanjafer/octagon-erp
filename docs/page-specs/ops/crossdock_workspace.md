---
page_id: "crossdock_workspace"
title_en: "Cross-dock Workspace"
title_ar: ""
domain: "ops"
navigation_group: "ops_inventory"
kind: PAGE
canonical_status: "PRIMARY"
canonical_home: "crossdock_workspace"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
domain_sources: ["modules/build09-dock-workspace.js", "platform/wms/index.mjs", "platform/wms/docks.mjs"]
tables_or_entities: ["wms_crossdock_matches", "wms_warehouse_tasks"]
test_sources: ["tests/build-09/dock-crossdock-domain.test.mjs", "tests/build-09/dock-workspaces-browser.test.mjs", "tests/build-09/cross-domain-lifecycles.test.mjs"]
...
- Page ID: `crossdock_workspace`; English: Cross-dock Workspace; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('crossdock_workspace')`; page-specific renderer is `modules/build09-dock-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Evaluate eligible inbound-to-outbound matches, approve a cross-dock move, and acknowledge its canonical posting.
...
- Renderer: `modules/build09-dock-workspace.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `crossdock-matches, locations` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:crossdock_evaluate, wms:crossdock_acknowledge_post.
...
- Renderer/domain source: `modules/build09-dock-workspace.js` → `crossdock.evaluateCrossDock/approveCrossDock/requestCrossDockPost/acknowledgeCrossDockPost/cancelCrossDock/listCrossDockMatches`.
- Persisted entities/tables: wms_crossdock_matches, wms_warehouse_tasks.
...
- `wms:crossdock_evaluate`
- `wms:crossdock_acknowledge_post`
...
- The page renders scoped records from `crossdock-matches, locations`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_crossdock_matches, wms_warehouse_tasks and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: dock_checkin, mobile_receiving.
- Downstream: staging_board, canonical inventory.
...
- English label: Cross-dock Workspace; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-crossdock_workspace-GAP-001** (P1, WORKFLOW) — evaluation, approval, canonical request, and acknowledgement are distinct domain steps and must not be documented as one button
- **PAGE-crossdock_workspace-GAP-002** (P1, ERROR_STATE) — no-match and cancelled-match evidence needs a direct browser row
...
- Canonical home: `crossdock_workspace` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-dock-workspace.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/dock-crossdock-domain.test.mjs`
- `tests/build-09/dock-workspaces-browser.test.mjs`
...
- `crossdock.evaluateCrossDock` → `approveCrossDock` → `requestCrossDockPost` → `acknowledgeCrossDockPost` → `cancelCrossDock` → `listCrossDockMatches`
...
- Cross-dock Workspace page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
