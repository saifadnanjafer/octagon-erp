...
renderer_type: "view"
renderer_sources: ["views/banking.html"]
...
- Page ID: `banking`
- English: Banking
- Arabic: البنوك والخزينة
- Domain: `finance`; navigation group: `finance_accounts`
- Route: switchPage('banking') / views/banking.html
- Page type: PAGE

# 2. Business Purpose

Banking and treasury — finance (risk: high, phase: phase6a)
...
User opens this page to work with the Banking workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/banking.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/banking.html
...
- UI → renderer: views/banking.html
...
The intended output is the page’s named Banking record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: البنوك والخزينة
- English label: Banking
...
- Canonical candidate: `banking` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `banking` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Banking workflow; the current 231/231 click audit is route activation evidence only.
...
- Banking workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
