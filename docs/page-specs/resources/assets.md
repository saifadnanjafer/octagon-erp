...
page_id: "assets"
title_en: "Assets"
title_ar: "الأصول والصيانة"
domain: "resources"
navigation_group: "resources_assets"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "assets"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
renderer_type: "view"
renderer_sources: ["views/assets.html"]
view_sources: ["views/assets.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/checkpoint-d-e/assets.test.mjs","tests/checkpoint-d-e/fleet.test.mjs","tests/checkpoint-d-e/maintenance.test.mjs","tests/checkpoint-f/atomicity_and_idempotency.test.mjs","tests/checkpoint-f/canonical_authority_coverage.test.mjs","tests/checkpoint-f/cross_domain_record_integrity.test.mjs","tests/checkpoint-g/canonical_cutover_controller.test.mjs","tests/checkpoint-g/disposable_backup_restore.test.mjs"]
...
- Page ID: `assets`
- English: Assets
- Arabic: الأصول والصيانة
- Domain: `resources`; navigation group: `resources_assets`
- Route: switchPage('assets') / views/assets.html
- Page type: PAGE

# 2. Business Purpose

Fixed Assets — workshop/maintenance (risk: medium, phase: phase6h)
...
User opens this page to work with the Assets workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/assets.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/assets.html
...
The intended output is the page’s named Assets record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: الأصول والصيانة
- English label: Assets
...
- Canonical candidate: `assets` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `assets` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Assets workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/checkpoint-d-e/assets.test.mjs, tests/checkpoint-d-e/fleet.test.mjs, tests/checkpoint-d-e/maintenance.test.mjs, tests/checkpoint-f/atomicity_and_idempotency.test.mjs, tests/checkpoint-f/canonical_authority_coverage.test.mjs, tests/checkpoint-f/cross_domain_record_integrity.test.mjs, tests/checkpoint-g/canonical_cutover_controller.test.mjs, tests/checkpoint-g/disposable_backup_restore.test.mjs
...
- Assets workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
