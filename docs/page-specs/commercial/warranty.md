...
renderer_type: "view"
renderer_sources: ["views/warranty.html"]
view_sources: ["views/warranty.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-03/commissions-warranty.test.mjs","tests/build-06/commercial-operations-lifecycle.test.mjs"]
...
- Page ID: `warranty`
- English: Warranty
- Arabic: الضمان والمرتجعات
- Domain: `commercial`; navigation group: `commercial_relationships`
- Route: switchPage('warranty') / views/warranty.html
- Page type: PAGE

# 2. Business Purpose

Warranty & RMA — workshop/operations (risk: medium, phase: phase6h)
...
User opens this page to work with the Warranty workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/warranty.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/warranty.html
...
- UI → renderer: views/warranty.html
...
The intended output is the page’s named Warranty record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: الضمان والمرتجعات
- English label: Warranty
...
- Canonical candidate: `warranty` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `warranty` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Warranty workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-03/commissions-warranty.test.mjs, tests/build-06/commercial-operations-lifecycle.test.mjs
...
- Warranty workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
