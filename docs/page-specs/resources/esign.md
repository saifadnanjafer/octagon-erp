...
page_id: "esign"
title_en: "Esign"
title_ar: "التوقيع الإلكتروني"
domain: "resources"
navigation_group: "resources_knowledge"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "esign"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
renderer_type: "view"
renderer_sources: ["views/esign.html"]
view_sources: ["views/esign.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-07/service-entitlement-signature-lifecycle.test.mjs","tests/build-10/cross-domain-scenarios.test.mjs","tests/checkpoint-d-e/projects_lifecycle.test.mjs"]
...
- Page ID: `esign`
- English: Esign
- Arabic: التوقيع الإلكتروني
- Domain: `resources`; navigation group: `resources_knowledge`
- Route: switchPage('esign') / views/esign.html
- Page type: PAGE

# 2. Business Purpose

E-Signatures — workshop/operations (risk: medium, phase: phase6h)
...
User opens this page to work with the Esign workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/esign.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/esign.html
...
The intended output is the page’s named Esign record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: التوقيع الإلكتروني
- English label: Esign
...
- Canonical candidate: `esign` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `esign` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Esign workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-07/service-entitlement-signature-lifecycle.test.mjs, tests/build-10/cross-domain-scenarios.test.mjs, tests/checkpoint-d-e/projects_lifecycle.test.mjs
...
- Esign workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
