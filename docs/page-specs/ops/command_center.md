...
renderer_sources: ["views/command_center.html","modules/build10-workspaces.js"]
view_sources: ["views/command_center.html"]
...
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-10/build10-workspaces-contract.test.mjs","tests/build-12/build12-browser.test.mjs","tests/build-12/build12-domain.test.mjs","tests/phase02/browser-live-evidence.test.mjs","tests/phase03/finance-browser-evidence.test.mjs","tests/workshop/browser-flows.test.mjs","tests/workshop/browser-harness.mjs","tests/workshop/command-center-contract.test.mjs"]
...
- Route: switchPage('command_center') / views/command_center.html
- Page type: DASHBOARD

# 2. Business Purpose

Command Center — manager/system (risk: high, phase: core)
...
User opens this page to work with the Command Center workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/command_center.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/command_center.html
...
- UI → renderer: views/command_center.html
...
The intended output is the page’s named Command Center record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: مركز القيادة
- English label: Command Center
...
- Canonical candidate: `command_center` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `command_center` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Command Center workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-10/build10-workspaces-contract.test.mjs, tests/build-12/build12-browser.test.mjs, tests/build-12/build12-domain.test.mjs, tests/phase02/browser-live-evidence.test.mjs, tests/phase03/finance-browser-evidence.test.mjs, tests/workshop/browser-flows.test.mjs, tests/workshop/browser-harness.mjs, tests/workshop/command-center-contract.test.mjs
...
- Command Center workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
