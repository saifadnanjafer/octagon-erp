...
tables_or_entities: ["wms_putaway_rules_v2", "wms_putaway_recommendations"]
test_sources: ["tests/build-09/putaway-workspaces-browser.test.mjs", "tests/build-09/build09r2-bespoke-contract.test.mjs"]
...
- Page ID: `putaway_rules`; English: Putaway Rules; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('putaway_rules')`; page-specific renderer is `modules/build09-putaway-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Configure deterministic placement rules and generate a governed putaway recommendation for inbound stock.
...
- Data loading: `putaway-rules` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:putaway_rule_create, wms:putaway_rule_update, wms:putaway_recommend.
...
- Renderer/domain source: `modules/build09-putaway-workspace.js` → `putaway.createPutawayRule/updatePutawayRule/recommendPutaway/listPutawayRules`.
- Persisted entities/tables: wms_putaway_rules_v2, wms_putaway_recommendations.
...
- `wms:putaway_rule_create`
- `wms:putaway_rule_update`
- `wms:putaway_recommend`
...
- The page renders scoped records from `putaway-rules`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_putaway_rules_v2, wms_putaway_recommendations and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: warehouse_topology, mobile_receiving.
- Downstream: putaway_task_queue, replenishment_proposals.
...
- English label: Putaway Rules; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-putaway_rules-GAP-001** (P1, WORKFLOW) — rule creation and recommendation are connected, but the catalog must retain explicit precedence/conflict behavior when several rules match
- **PAGE-putaway_rules-GAP-002** (P1, TEST) — prove recommendation selection against restricted, blocked, capacity, and quality-ineligible locations
...
- Canonical home: `putaway_rules` unless a later owner decision explicitly changes the navigation contract.
...
- `putaway.createPutawayRule` → `updatePutawayRule` → `recommendPutaway` → `listPutawayRules`
...
- Putaway Rules page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
