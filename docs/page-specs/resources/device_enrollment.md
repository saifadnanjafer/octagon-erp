---
page_id: "device_enrollment"
title_en: "Device Enrollment"
title_ar: "تسجيل الأجهزة"
domain: "resources"
navigation_group: "resources_devices"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "device_enrollment"
parent_page: null
aliases: []
...
- Page ID: `device_enrollment`
- English: Device Enrollment
- Arabic: تسجيل الأجهزة
- Domain: `resources`; navigation group: `resources_devices`
- Route: switchPage('device_enrollment') / views/device_enrollment.html
- Page type: PAGE

# 2. Business Purpose

Device enrollment — iot/devices (risk: high, phase: build10)
...
User opens this page to work with the Device Enrollment workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/device_enrollment.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Device Enrollment record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: تسجيل الأجهزة
- English label: Device Enrollment
...
- Canonical candidate: `device_enrollment` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `device_enrollment` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Device Enrollment workflow; the current 231/231 click audit is route activation evidence only.
...
- Device Enrollment workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
