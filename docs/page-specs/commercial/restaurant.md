...
renderer_type: "view"
renderer_sources: ["views/restaurant.html"]
view_sources: ["views/restaurant.html"]
api_sources: []
...
- Route: switchPage('restaurant') / views/restaurant.html
- Page type: PAGE

# 2. Business Purpose

Restaurant vertical — vertical/restaurant (risk: medium, phase: phase6h)
...
User opens this page to work with the Restaurant workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/restaurant.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/restaurant.html
...
- UI → renderer: views/restaurant.html
...
The intended output is the page’s named Restaurant record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Canonical candidate: `restaurant` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `restaurant` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Restaurant workflow; the current 231/231 click audit is route activation evidence only.
...
- Restaurant workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
