...
- Page ID: `intercompany_reconciliation`; English: Intercompany Reconciliation; domain: `finance`; navigation group: `finance_intercompany`.
- Route: `switchPage('intercompany_reconciliation')`; view: `views/intercompany_reconciliation.html`; renderer evidence: `app.js`.
- Canonical status: PRIMARY; this page is a primary navigation destination, not an embedded tab.

# 2. Business Purpose

An intercompany accountant opens Reconciliation to resolve matched, mismatched, and accepted cross-company transactions with audit evidence.
...
USER OPENS THIS PAGE TO: An intercompany accountant opens Reconciliation to resolve matched, mismatched, and accepted cross-company transactions with audit evidence.

# 5. AS-IS Runtime Surface

- View: `views/intercompany_reconciliation.html`; renderer: `app.js generic/legacy surface`.
...
- Primary actions:
- Match/accept/reject/reconcile -> page action/API NOT VERIFIED — permission: server authorization required; persistence: Intercompany operations owns agreement, mismatch, and reconciliation workflow; Finance owns each company ledger posting.
...
An intercompany accountant opens Reconciliation to resolve matched, mismatched, and accepted cross-company transactions with audit evidence. Expected output: scoped records, decision state, durable source reference, report, alert, or device/session state as appropriate. Exact persisted result is NOT VERIFIED for the page-specific surface.
...
- English: Intercompany Reconciliation; Arabic: · التسوية والمطابقة.
...
An intercompany accountant opens Reconciliation to resolve matched, mismatched, and accepted cross-company transactions with audit evidence. The target contract is a scoped, page-specific workflow that loads authoritative data, exposes lifecycle-valid actions, persists through **Intercompany operations owns agreement, mismatch, and reconciliation workflow; Finance owns each company ledger posting.**, produces a durable result/reference, and makes missing, denied, stale, validation, and server-error states explicit.

# 17. Identified Gaps

- **intercompany_reconciliation-W4-01** (P1, WORKFLOW_DISCONNECT) — The intercompany domain and tests exist, but page-level transaction-to-mismatch-to-reconciliation evidence is not attributed. **Required next step:** Prove the complete pair-scoped lifecycle and its Finance posting/lineage references.
- **intercompany_reconciliation-W4-02** (P2, ISOLATION_GAP) — Company-pair access and cross-company authorization are not proven by navigation evidence. **Required next step:** Add role and company-pair isolation tests.
...
- For this page: An intercompany accountant opens Reconciliation to resolve matched, mismatched, and accepted cross-company transactions with audit evidence.
