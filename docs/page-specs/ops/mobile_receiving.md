...
domain_sources: ["modules/build09-mobile-receiving.js", "platform/wms/index.mjs", "platform/wms/receiving.mjs"]
tables_or_entities: ["wms_receiving_sessions", "wms_receiving_lines", "wms_receiving_discrepancies"]
test_sources: ["tests/build-09/mobile-receiving-domain.test.mjs", "tests/build-09/mobile-receiving-picking-browser.test.mjs", "tests/build-09/operational-browser-chromium.test.mjs", "tests/build-09/build09r-form-contract.test.mjs"]
...
- Page ID: `mobile_receiving`; English: Mobile Receiving; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('mobile_receiving')`; page-specific renderer is `modules/build09-mobile-receiving.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Scan an inbound reference and products, identify discrepancies, review, request canonical posting, acknowledge Inventory, and complete putaway-pending receipt.
...
- Renderer: `modules/build09-mobile-receiving.js`; the module registers a page-specific BUILD-09 workspace/override rather than relying on the generic table shell.
- Data loading: `direct session actions; receiving-sessions is the authoritative list resource` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:receiving_start, wms:receiving_scan_reference, wms:receiving_scan_product, wms:receiving_review, wms:receiving_request_post, wms:receiving_acknowledge_post, wms:receiving_complete.
...
- Renderer/domain source: `modules/build09-mobile-receiving.js` → `receiving.startReceiving/scanReceivingReference/scanReceivingProduct/reviewReceiving/requestReceivingPost/acknowledgeReceivingPost/completeReceiving`.
- Persisted entities/tables: wms_receiving_sessions, wms_receiving_lines, wms_receiving_discrepancies.
...
- `wms:receiving_start`
- `wms:receiving_scan_reference`
- `wms:receiving_scan_product`
- `wms:receiving_review`
- `wms:receiving_request_post`
- `wms:receiving_acknowledge_post`
- `wms:receiving_complete`
...
- The page renders scoped records from `direct session actions; receiving-sessions is the authoritative list resource`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_receiving_sessions, wms_receiving_lines, wms_receiving_discrepancies and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: purchase/transfer/return reference, warehouse_topology.
- Downstream: receiving_discrepancies, putaway_task_queue, canonical inventory picking.
...
- English label: Mobile Receiving; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-mobile_receiving-GAP-001** (P1, WORKFLOW) — direct browser proof must cover the full scan-to-canonical-posting handoff including over/under receipt
- **PAGE-mobile_receiving-GAP-002** (P1, RESPONSIVE) — scanner-sized controls and focus behavior need a bounded mobile viewport assertion
...
- Canonical home: `mobile_receiving` unless a later owner decision explicitly changes the navigation contract.
...
- STATIC/CONTRACT: `modules/build09-mobile-receiving.js`, `platform/api/build09.mjs`, `platform/wms/index.mjs`.
- BUILD-09 test sources:
- `tests/build-09/mobile-receiving-domain.test.mjs`
- `tests/build-09/mobile-receiving-picking-browser.test.mjs`
- `tests/build-09/operational-browser-chromium.test.mjs`
- `tests/build-09/build09r-form-contract.test.mjs`
...
- `receiving.startReceiving` → `scanReceivingReference` → `scanReceivingProduct` → `reviewReceiving` → `requestReceivingPost` → `acknowledgeReceivingPost` → `completeReceiving`
...
- Mobile Receiving page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
