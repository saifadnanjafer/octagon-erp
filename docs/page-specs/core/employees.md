...
page_id: "employees"
title_en: "Employees"
title_ar: "الموظفين والأرصدة"
domain: "core"
navigation_group: "core_daily"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "employees"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
renderer_type: "view"
renderer_sources: ["views/employees.html"]
view_sources: ["views/employees.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/checkpoint-f/canonical_authority_coverage.test.mjs","tests/checkpoint-g/canonical_cutover_controller.test.mjs","tests/checkpoint-h/http_legacy_writer_refusal.test.mjs","tests/navigation/workshop-pack-setup-to-readiness.test.mjs","tests/phase02/browser-live-evidence.test.mjs","tests/phase02/runtime-adversarial.test.mjs","tests/phase02/runtime-integration.test.mjs","tests/phase02/security-suite.test.mjs"]
...
- Page ID: `employees`
- English: Employees
- Arabic: الموظفين والأرصدة
- Domain: `core`; navigation group: `core_daily`
- Route: switchPage('employees') / views/employees.html
- Page type: PAGE

# 2. Business Purpose

Employees and balances — hr/payroll (risk: high, phase: core)
...
User opens this page to work with the Employees workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/employees.html. Static source counts at baseline: 2 headings, 3 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/employees.html
...
- UI → renderer: views/employees.html
...
- UI label: تحديث; handler/action ID: renderEmployeesTable(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
- UI label: تصدير البيانات; handler/action ID: exportData(); permission and persisted result: NOT VERIFIED unless a page-specific API is listed above.
...
The intended output is the page’s named Employees record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: الموظفين والأرصدة
- English label: Employees
...
- **PAGE-employees-GAP-003** (P1, API) — No literal backend API path was verified in the inspected page-specific source; server capability remains NOT VERIFIED.
...
- Canonical candidate: `employees` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `employees` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Employees workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/checkpoint-f/canonical_authority_coverage.test.mjs, tests/checkpoint-g/canonical_cutover_controller.test.mjs, tests/checkpoint-h/http_legacy_writer_refusal.test.mjs, tests/navigation/workshop-pack-setup-to-readiness.test.mjs, tests/phase02/browser-live-evidence.test.mjs, tests/phase02/runtime-adversarial.test.mjs, tests/phase02/runtime-integration.test.mjs, tests/phase02/security-suite.test.mjs
...
- Employees workspace presentation and its explicitly verified page-specific actions: addEmployee(), renderEmployeesTable(), exportData().
