...
Six purpose-built workspace groups replacing the generic table+dialog shell on the highest-value
BUILD-09 operational pages, plus a shared workspace kernel, three defect fixes found along the
way, and 13 new real-Chromium tests.
...
primitives, so six groups read as one product instead of six separately-grown pages.
...
**16 of 32** BUILD-09 pages are now purpose-built (14 this session + the 2 mobile workspaces from
the prior session). The remaining 16 are pinned by name in
...
npm.cmd run test:build-09     51/51   (35 baseline preserved + 16 new)
...
13 new real-Chromium flows driven through visible controls only (no `page.evaluate(fetch(...))`
...
16 pages still on the generic shell, pinned in the contract test: `warehouse_topology`,
`zone_bin_management`, `putaway_rules`, `putaway_task_queue`, `replenishment_rules`,
`replenishment_proposals`, `receiving_discrepancies`, `pick_task_queue`, `dock_schedule`,
`dock_checkin`, `staging_board`, `crossdock_workspace`, `expiration_queue`,
