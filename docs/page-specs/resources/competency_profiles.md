---
page_id: "competency_profiles"
title_en: "Competency Profiles"
title_ar: "ملفات الكفاءات"
domain: "resources"
navigation_group: "resources_people"
kind: SETUP
canonical_status: PRIMARY
canonical_home: "competency_profiles"
parent_page: null
aliases: []
...
- Page ID: `competency_profiles`
- English: Competency Profiles
- Arabic: ملفات الكفاءات
- Domain: `resources`; navigation group: `resources_people`
- Route: switchPage('competency_profiles') / views/competency_profiles.html
- Page type: SETUP

# 2. Business Purpose

Competency profiles — build12/competencies (risk: medium, phase: build12)
...
User opens this page to work with the Competency Profiles workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/competency_profiles.html. Static source counts at baseline: 2 headings, 3 button tags, 2 input/select/textarea controls, 1 tables, 1 forms, and 2 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Competency Profiles record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: ملفات الكفاءات
- English label: Competency Profiles
...
The Competency Profiles workspace should give its governed users a page-specific way to complete the named business task: load scoped records, accept governed inputs where needed, execute authorized actions, persist the resulting state through the domain authority, and expose explicit loading/empty/denied/validation/server-error/success states. Exact fields and lifecycle transitions remain **NOT VERIFIED** where the baseline source does not define them.

# 17. Identified Gaps

- **PAGE-competency_profiles-GAP-004** (P1, PERMISSION) — The review inventory flags this commercial/SaaS page as lacking a matching entitlement mapping in the disposable review database.
- **PAGE-competency_profiles-GAP-005** (P2, TEST) — Documentation contradiction: NAVIGATION_FORENSIC_REPORT.json records app.js switchPage, while the module registry also contains this page. Reconcile renderer ownership after Product Recovery.
...
- Canonical candidate: `competency_profiles` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `competency_profiles` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Competency Profiles workflow; the current 231/231 click audit is route activation evidence only.
...
- Competency Profiles workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED, NOT VERIFIED.
