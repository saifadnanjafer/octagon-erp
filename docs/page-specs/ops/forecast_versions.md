---
page_id: "forecast_versions"
title_en: "Forecast Versions"
title_ar: "· إصدارات التنبؤ"
domain: "ops"
navigation_group: "ops_planning"
kind: ANALYTICS
canonical_status: PRIMARY
canonical_home: "forecast_versions"
parent_page: null
aliases: []
...
renderer_type: "view"
renderer_sources: ["views/forecast_versions.html"]
view_sources: ["views/forecast_versions.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-08/build08-page-contract.test.mjs","tests/build-08/planning-browser-chromium.test.mjs","tests/build-08/platform-governance-contract.test.mjs"]
...
- Page ID: `forecast_versions`
- English: Forecast Versions
- Arabic: · إصدارات التنبؤ
- Domain: `ops`; navigation group: `ops_planning`
- Route: switchPage('forecast_versions') / views/forecast_versions.html
- Page type: ANALYTICS

# 2. Business Purpose

Forecast versions — planning (risk: high, phase: build08)
...
User opens this page to work with the Forecast Versions workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/forecast_versions.html. Static source counts at baseline: 1 headings, 0 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/forecast_versions.html
...
- UI → renderer: views/forecast_versions.html
...
The intended output is the page’s named Forecast Versions record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: · إصدارات التنبؤ
- English label: Forecast Versions
...
- Canonical candidate: `forecast_versions` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `forecast_versions` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Forecast Versions workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-08/build08-page-contract.test.mjs, tests/build-08/planning-browser-chromium.test.mjs, tests/build-08/platform-governance-contract.test.mjs
...
- Forecast Versions workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
