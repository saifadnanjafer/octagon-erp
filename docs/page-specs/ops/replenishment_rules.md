...
tables_or_entities: ["wms_replenishment_rules_v2"]
test_sources: ["tests/build-09/putaway-workspaces-browser.test.mjs", "tests/build-09/wms-foundation-domain.test.mjs"]
...
- Page ID: `replenishment_rules`; English: Replenishment Rules; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('replenishment_rules')`; page-specific renderer is `modules/build09-putaway-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Set min/max or demand-driven replenishment policies for warehouse locations.
...
- Data loading: `replenishment-rules` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:replenishment_rule_create.
...
- Renderer/domain source: `modules/build09-putaway-workspace.js` → `replenishment.createReplenishmentRule/listReplenishmentRules`.
- Persisted entities/tables: wms_replenishment_rules_v2.
...
- The page renders scoped records from `replenishment-rules`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_replenishment_rules_v2 and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: warehouse_topology.
- Downstream: replenishment_proposals.
...
- English label: Replenishment Rules; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-replenishment_rules-GAP-001** (P1, ACTION) — the visible form is a rule-creation surface
- **PAGE-replenishment_rules-GAP-002** (P1, WORKFLOW) — update/activation/deactivation semantics are domain-supported but not fully exposed
- **PAGE-replenishment_rules-GAP-003** (P1, TEST) — prove rules are filtered to the selected warehouse and company
...
- Canonical home: `replenishment_rules` unless a later owner decision explicitly changes the navigation contract.
...
- `replenishment.createReplenishmentRule` → `listReplenishmentRules`
...
- Replenishment Rules page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
