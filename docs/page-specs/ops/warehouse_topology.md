...
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-09/build09-workspaces-contract.test.mjs","tests/build-09/build09r2-bespoke-contract.test.mjs","tests/build-09/operational-32-page-matrix-chromium.test.mjs","tests/build-09/operational-runtime-context-chromium.test.mjs","tests/build-09/operational-topology-chromium.test.mjs","tests/build-09/topology-workspaces-browser.test.mjs"]
...
- Page ID: `warehouse_topology`
- English: Warehouse Topology
- Arabic: هيكل المستودع
- Domain: `ops`; navigation group: `ops_inventory`
- Route: switchPage('warehouse_topology') / views/warehouse_topology.html
- Page type: PAGE

# 2. Business Purpose

Warehouse topology — workshop/inventory (risk: medium, phase: build09)
...
User opens this page to work with the Warehouse Topology workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/warehouse_topology.html. Static source counts at baseline: 5 headings, 2 button tags, 0 input/select/textarea controls, 0 tables, 2 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Warehouse Topology record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.

# 10. Workflow Position

- Upstream: NOT VERIFIED from explicit workflow metadata.
- Downstream: `zone_bin_management`
...
- Arabic label: هيكل المستودع
- English label: Warehouse Topology
...
- Canonical candidate: `warehouse_topology` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `warehouse_topology` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Warehouse Topology workflow; the current 231/231 click audit is route activation evidence only.
...
- Warehouse Topology workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED.
