---
page_id: "expiration_queue"
title_en: "Expiration Queue"
title_ar: "طابور تواريخ الانتهاء"
domain: "ops"
navigation_group: "ops_warehouse_traceability"
kind: QUEUE
canonical_status: PRIMARY
canonical_home: "expiration_queue"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
view_sources: []
api_sources: []
domain_sources: ["modules/build09-expiration-workspace.js","services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/build-09/build09-workspaces-contract.test.mjs","tests/build-09/build09r2-bespoke-contract.test.mjs","tests/build-09/expiration-queue-browser.test.mjs","tests/build-09/operational-32-page-matrix-chromium.test.mjs"]
...
- Page ID: `expiration_queue`
- English: Expiration Queue
- Arabic: طابور تواريخ الانتهاء
- Domain: `ops`; navigation group: `ops_warehouse_traceability`
- Route: switchPage('expiration_queue') / views/expiration_queue.html
- Page type: QUEUE

# 2. Business Purpose

Expiration queue — workshop/quality (risk: high, phase: build09)
...
User opens this page to work with the Expiration Queue workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/expiration_queue.html. Static source counts at baseline: 0 headings, 2 button tags, 3 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Expiration Queue record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: طابور تواريخ الانتهاء
- English label: Expiration Queue
...
- Canonical candidate: `expiration_queue` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `expiration_queue` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Expiration Queue workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/build-09/build09-workspaces-contract.test.mjs, tests/build-09/build09r2-bespoke-contract.test.mjs, tests/build-09/expiration-queue-browser.test.mjs, tests/build-09/operational-32-page-matrix-chromium.test.mjs
...
- Expiration Queue workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED, NOT VERIFIED.
