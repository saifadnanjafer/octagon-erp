...
tables_or_entities: ["wms_docks_v2", "wms_dock_appointments_v2"]
...
- Page ID: `dock_schedule`; English: Dock Schedule; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('dock_schedule')`; page-specific renderer is `modules/build09-dock-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Schedule inbound/outbound appointments against active docks and show expected arrival, capacity, and status.
...
- Data loading: `docks, dock-appointments` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:dock_appointment_create, wms:dock_cancel.
...
- Renderer/domain source: `modules/build09-dock-workspace.js` → `docks.createDock/createDockAppointment/cancelDockAppointment/listDocks/listDockAppointments`.
- Persisted entities/tables: wms_docks_v2, wms_dock_appointments_v2.
...
- The page renders scoped records from `docks, dock-appointments`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_docks_v2, wms_dock_appointments_v2 and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: warehouse_topology.
- Downstream: dock_checkin, staging_board.
...
- English label: Dock Schedule; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-dock_schedule-GAP-001** (P1, SCOPE) — appointment creation must validate dock and staging location belong to the active warehouse
- **PAGE-dock_schedule-GAP-002** (P1, TEST) — prove conflict/capacity and cancellation behavior with non-empty fixtures
...
- Canonical home: `dock_schedule` unless a later owner decision explicitly changes the navigation contract.
...
- `docks.createDock` → `createDockAppointment` → `cancelDockAppointment` → `listDocks` → `listDockAppointments`
...
- Dock Schedule page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
