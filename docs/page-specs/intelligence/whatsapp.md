...
page_id: "whatsapp"
title_en: "Whatsapp"
title_ar: "استقبال واتساب"
domain: "intelligence"
navigation_group: "intelligence_core"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "whatsapp"
parent_page: null
aliases: []
...
renderer_type: "view"
renderer_sources: ["views/whatsapp.html"]
view_sources: ["views/whatsapp.html"]
api_sources: []
domain_sources: ["services/permissionService.js"]
tables_or_entities: []
test_sources: ["docs/navigation/NAVIGATION_FORENSIC_REPORT.json","docs/autopilot/evidence/NAVIGATION-RECOVERY-1-click-audit-all.json","tests/phase02/collaboration-files-jobs.test.mjs","tests/phase02/runtime-adversarial.test.mjs","tests/phase02/security-suite.test.mjs","tests/phase02/settings-policies.test.mjs"]
...
- Page ID: `whatsapp`
- English: Whatsapp
- Arabic: استقبال واتساب
- Domain: `intelligence`; navigation group: `intelligence_core`
- Route: switchPage('whatsapp') / views/whatsapp.html
- Page type: PAGE

# 2. Business Purpose

WhatsApp control — ai/customer_comms (risk: high, phase: core)
...
User opens this page to work with the Whatsapp workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/whatsapp.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/whatsapp.html
...
- UI → renderer: views/whatsapp.html
...
The intended output is the page’s named Whatsapp record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: استقبال واتساب
- English label: Whatsapp
...
- **PAGE-whatsapp-GAP-002** (P1, ACTION) — No meaningful primary action path was verified in the inspected source.
- **PAGE-whatsapp-GAP-003** (P1, API) — No literal backend API path was verified in the inspected page-specific source; server capability remains NOT VERIFIED.
...
- Canonical candidate: `whatsapp` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `whatsapp` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Whatsapp workflow; the current 231/231 click audit is route activation evidence only.
...
- **API / DOMAIN:** tests/phase02/collaboration-files-jobs.test.mjs, tests/phase02/runtime-adversarial.test.mjs, tests/phase02/security-suite.test.mjs, tests/phase02/settings-policies.test.mjs
...
- Whatsapp workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
