...
renderer_type: "view"
renderer_sources: ["views/budgeting.html"]
...
- Route: switchPage('budgeting') / views/budgeting.html
- Page type: PAGE

# 2. Business Purpose

Budgeting — finance (risk: high, phase: phase6e)
...
User opens this page to work with the Budgeting workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/budgeting.html. Static source counts at baseline: 1 headings, 0 button tags, 0 input/select/textarea controls, 0 tables, 0 forms, and 0 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.

- Renderer evidence: views/budgeting.html
...
- UI → renderer: views/budgeting.html
...
The intended output is the page’s named Budgeting record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: الموازنات
- English label: Budgeting
...
- Canonical candidate: `budgeting` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `budgeting` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Budgeting workflow; the current 231/231 click audit is route activation evidence only.
...
- Budgeting workspace presentation and its explicitly verified page-specific actions: NOT VERIFIED.
