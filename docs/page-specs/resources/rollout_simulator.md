---
page_id: "rollout_simulator"
title_en: "Rollout Simulator"
title_ar: "محاكي النشر"
domain: "resources"
navigation_group: "resources_devices"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "rollout_simulator"
parent_page: null
aliases: []
...
- Page ID: `rollout_simulator`
- English: Rollout Simulator
- Arabic: محاكي النشر
- Domain: `resources`; navigation group: `resources_devices`
- Route: switchPage('rollout_simulator') / views/rollout_simulator.html
- Page type: PAGE

# 2. Business Purpose

Rollout simulator — iot/firmware (risk: high, phase: build10)
...
User opens this page to work with the Rollout Simulator workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/rollout_simulator.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Rollout Simulator record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: محاكي النشر
- English label: Rollout Simulator
...
- Canonical candidate: `rollout_simulator` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `rollout_simulator` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Rollout Simulator workflow; the current 231/231 click audit is route activation evidence only.
...
- Rollout Simulator workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
