---
page_id: "device_detail"
title_en: "Device Detail"
title_ar: "تفاصيل الجهاز"
domain: "resources"
navigation_group: "resources_devices"
kind: PAGE
canonical_status: PRIMARY
canonical_home: "device_detail"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
- Page ID: `device_detail`
- English: Device Detail
- Arabic: تفاصيل الجهاز
- Domain: `resources`; navigation group: `resources_devices`
- Route: switchPage('device_detail') / views/device_detail.html
- Page type: PAGE

# 2. Business Purpose

Device detail — iot/devices (risk: low, phase: build10)
...
User opens this page to work with the Device Detail workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/device_detail.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Device Detail record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: تفاصيل الجهاز
- English label: Device Detail
...
- Canonical candidate: `device_detail` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `device_detail` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Device Detail workflow; the current 231/231 click audit is route activation evidence only.
...
- Device Detail workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
