---
page_id: "device_registry"
title_en: "Device Registry"
title_ar: "سجل الأجهزة"
domain: "resources"
navigation_group: "resources_devices"
kind: SETUP
canonical_status: PRIMARY
canonical_home: "device_registry"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
- Page ID: `device_registry`
- English: Device Registry
- Arabic: سجل الأجهزة
- Domain: `resources`; navigation group: `resources_devices`
- Route: switchPage('device_registry') / views/device_registry.html
- Page type: SETUP

# 2. Business Purpose

Device registry — iot/devices (risk: medium, phase: build10)
...
User opens this page to work with the Device Registry workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/device_registry.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Device Registry record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: سجل الأجهزة
- English label: Device Registry
...
- Canonical candidate: `device_registry` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `device_registry` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Device Registry workflow; the current 231/231 click audit is route activation evidence only.
...
- Device Registry workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
