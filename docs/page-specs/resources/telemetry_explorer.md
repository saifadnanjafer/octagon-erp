---
page_id: "telemetry_explorer"
title_en: "Telemetry Explorer"
title_ar: "مستكشف القياسات"
domain: "resources"
navigation_group: "resources_devices"
kind: ANALYTICS
canonical_status: PRIMARY
canonical_home: "telemetry_explorer"
parent_page: null
aliases: []
roles: ["workshop.user"]
...
- Page ID: `telemetry_explorer`
- English: Telemetry Explorer
- Arabic: مستكشف القياسات
- Domain: `resources`; navigation group: `resources_devices`
- Route: switchPage('telemetry_explorer') / views/telemetry_explorer.html
- Page type: ANALYTICS

# 2. Business Purpose

Telemetry explorer — iot/telemetry (risk: low, phase: build10)
...
User opens this page to work with the Telemetry Explorer workspace. The exact business completion goal is supported by the review inventory purpose statement above.

# 5. AS-IS Runtime Surface

The registered view is views/telemetry_explorer.html. Static source counts at baseline: 6 headings, 5 button tags, 1 input/select/textarea controls, 1 tables, 0 forms, and 6 literal /api/v1 calls. Dynamic content not visible in static source is NOT VERIFIED.
...
The intended output is the page’s named Telemetry Explorer record, decision, view, or operational state. Exact persisted object: NOT VERIFIED.
...
- Arabic label: مستكشف القياسات
- English label: Telemetry Explorer
...
- Canonical candidate: `telemetry_explorer` unless a later owner decision records another home.

# 19. Acceptance Criteria

- A user with the declared permission can open `telemetry_explorer` through the visible navigation and see a non-error page-specific surface.
...
- A scoped browser test proves the named Telemetry Explorer workflow; the current 231/231 click audit is route activation evidence only.
...
- Telemetry Explorer workspace presentation and its explicitly verified page-specific actions: ${pageKey}:view, ${pageKey}:edit, ${pageKey}:create, OctagonBuild10.exportCsv(, ${pageKey}:view.
