...
tables_or_entities: ["wms_putaway_recommendations", "wms_putaway_recommendation_lines", "wms_warehouse_tasks"]
test_sources: ["tests/build-09/putaway-workspaces-browser.test.mjs", "tests/build-09/operational-32-page-matrix-chromium.test.mjs", "tests/build-09/cross-domain-lifecycles.test.mjs"]
...
- Page ID: `putaway_task_queue`; English: Putaway Task Queue; domain: `ops`; navigation group: `ops_inventory`.
- Route: `switchPage('putaway_task_queue')`; page-specific renderer is `modules/build09-putaway-workspace.js`.
- Canonical status: PRIMARY. Embedded tabs and compatibility aliases are not treated as separate pages.

# 2. Business Purpose

Execute assigned putaway tasks by scanning source and destination, then hand off the stock movement to canonical Inventory.
...
- Data loading: `putaway-queue, tasks` through the governed Build-09 API/query boundary; mobile picking additionally uses the explicit pick-task GET route.
- Primary actions exposed by this page: wms:task_scan_source, wms:task_scan_destination, wms:task_request_canonical, wms:task_acknowledge_canonical.
...
- Renderer/domain source: `modules/build09-putaway-workspace.js` → `putaway.listPutawayQueue/listWarehouseTasks/scanTaskSource/scanTaskDestination/requestCanonicalMovement/acknowledgeCanonicalMovement`.
- Persisted entities/tables: wms_putaway_recommendations, wms_putaway_recommendation_lines, wms_warehouse_tasks.
...
- `wms:task_scan_source`
- `wms:task_scan_destination`
- `wms:task_request_canonical`
- `wms:task_acknowledge_canonical`
...
- The page renders scoped records from `putaway-queue, tasks`, lifecycle/status badges, quantities and operational KPIs where applicable.
- Mutations persist into wms_putaway_recommendations, wms_putaway_recommendation_lines, wms_warehouse_tasks and refresh the query result; canonical handoff pages expose the returned request/result reference without claiming ownership of the canonical side effect.

# 10. Workflow Position

- Upstream: putaway_rules, mobile_receiving.
- Downstream: canonical inventory stock move, location stock.
...
- English label: Putaway Task Queue; Arabic label is owned by the navigation/module localization layer and must remain paired with the visible English action/state label.
...
- **PAGE-putaway_task_queue-GAP-001** (P1, PERMISSION) — task visibility and mutation must remain warehouse-scoped for viewer/operator roles
- **PAGE-putaway_task_queue-GAP-002** (P1, WORKFLOW) — canonical posting acknowledgement needs a direct browser proof with an actual completed move ID
...
- Canonical home: `putaway_task_queue` unless a later owner decision explicitly changes the navigation contract.
...
- Putaway Task Queue page-specific presentation, validation, and lifecycle orchestration at the cited module boundary.
